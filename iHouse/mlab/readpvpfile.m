...
if isempty(errorstring)
    ...
    case 1 % PVP_FILE_TYPE, used in HyPerCol::exitRunLoop
        ...
        case 2 % PVP_ACT_FILE_TYPE % Compressed for spiking; I'm not using yet
            ...
            averageFiringRate = (1000 .* sum(firingRate(:))) ./ (numframes * hdr.nxGlobal * hdr.nyGlobal);
            firingRate = (1000 * firingRate) ./ numframes;
            latPrintImage(firingRate, 0, 0, output_path, -1, ['firing\_rate:',num2str(averageFiringRate, '%f')]);
            if eq(MOVIE_FLAG,1)
