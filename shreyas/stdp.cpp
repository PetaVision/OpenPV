...
//   new PV::RuleConn("L2 to L1",     hc, l2,     l1, CHANNEL_EXC);
   ...
   int locX = 8;  // image ON at 32 (so rule fires ON 33)
   int locY = 0;   // image ON
   ...
   PV::PVLayerProbe * ptprobeI = new PV::PointProbe(8, locY, 0, "L1:x=8 f=0");
...
//   PV::PVLayerProbe * ptprobe7 = new PV::PointProbe(61, locY, 7, "L1:x=61 f=7");
   ...
   // connection probes
   PV::ConnectionProbe * cProbe0 = new PV::ConnectionProbe(2*4 + 0);
   PV::ConnectionProbe * cProbe1 = new PV::ConnectionProbe(2*4 + 1);
...
//   l1Inh->insertProbe(probe0);
   ...
   r_l1->insertProbe(cProbe0); // prints out the weights (x,y,f) at that neuron
