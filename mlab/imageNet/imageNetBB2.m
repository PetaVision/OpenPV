function [mask_image, scale_x, scale_y] = ...
  ...
  for i_bndbox = 1 : num_bndbox
    ...
    if GRABCUT_FLAG
      ...
      base_pathname = strRemoveExtension(xml_file);
      mask_pathname = strcat(base_pathname, ".jpg");
      system(["/usr/local/bin/grabcut ", ...
	      num2str(num_grabcut_iterations), ...
	      original_pathname, ...
	      mask_pathname, ...
	      num2str(BB_xmin), num2str(BB_ymin), num2str(BB_xmax), num2str(BB_ymax)]);
      BB_mask_tmp = imread(mask_pathname);
    else
