function [ layer_index ] = pvp_image2layer( spikes, image_index, timesteps, use_max, pvp_order )
...
if use_max && row_scale * col_scale * NFEATURES > 1
   ...
   irow_layer = squeeze( irow_layer( :, max_index ) );
   jcol_layer = squeeze( jcol_layer( :, max_index ) );
   f_layer = squeeze( f_layer( :, max_index ) );
end
