...
input_path = '/nh/home/gkenyon/workspace/PetaVision/mlab/amoebaGen/128_png/2/';
...
end_time = inf;
stim_begin_time = 200.0;  % times (msec) before this and after begin_time can be used to calculate background
stim_end_time = 800.0;
stim_exclude_time = 50.0;
bin_size = 4.0;  % (msec) used for all rate calculations
analysis_start_time = 100.0;
...
read_spikes = 2:N_LAYERS;  % list of spiking layers whose spike train are to be analyzed
num_layers = N_LAYERS;
...
image_filename = [input_path 't/tar_0001_a.png'];
target_filename{1} = [input_path 'a/tar_0001_a.png'];
...
for layer = read_spikes;
    ...
    if plot_raster2
        ...
        %%fig_list = [fig_list; fig_tmp];
	subplot(2, 1, 1);
        ah = axis([analysis_start_step analysis_stop_step 0 num_neurons(layer)])
	axis normal
        hold on
        [spike_time, spike_id] = ...
    ...
    for i_target = 1:num_targets
	  ...
	  rate_array{layer}(1,target_ndx{layer, i_target});
...
%         xcorr( ave_target_tmp', [], max_lag, 'unbiased' );
    ave_target_tmp = ...
        zeros(num_stim_steps,1);
    ave_target_tmp(:,1) = ...
        full(ave_target{layer,i_target}(stim_steps));
    [target_xcorr, ...
     target_xcorr_std, ...
     target_xcorr_dist, ...
     target_xcorr_mean, ...
     target_xcorr_lags, ...
     target_xcorr_figs] = ...
     pvp_xcorr( ave_target_tmp, ...
     ave_target_tmp, ...
     max_lag, ...
     1, [num_stim_steps, 1], ...
     1, [num_stim_steps, 1], ...
     1, 1 );
    target_xcorr = squeeze( target_xcorr(1,1,:) );
    target_xcorr_flip = ...
        flipdim( target_xcorr, 1 );
    target_xcorr = [ target_xcorr_flip; target_xcorr(2:end) ];
    target_xcorr = ( target_xcorr - mean(ave_target_tmp)^2 ) / ...
     (mean(ave_target_tmp)+(mean(ave_target_tmp)==0))^2;
    target_xcorr(max_lag + 1) = 0;
    lh_target = plot((-max_lag:max_lag)*dt, squeeze(target_xcorr), '-r');
...
%         xcorr( ave_clutter_tmp', [], max_lag, 'unbiased' );
    ave_clutter_tmp = ...
        zeros(num_stim_steps,1);
    ave_clutter_tmp(:,1) = ...
        full(ave_clutter{layer,1}(stim_steps));
    [clutter_xcorr, ...
     clutter_xcorr_std, ...
     clutter_xcorr_dist, ...
     clutter_xcorr_mean, ...
     clutter_xcorr_lags, ...
     clutter_xcorr_figs] = ...
     pvp_xcorr( ave_clutter_tmp, ...
            ave_clutter_tmp, ...
            max_lag, ...
            1, [num_stim_steps, 1], ...
            1, [num_stim_steps, 1], ...
            1, 1 );        
    clutter_xcorr = squeeze( clutter_xcorr(1,1,:) );
    clutter_xcorr_flip = ...
        flipdim( clutter_xcorr, 1 );
    clutter_xcorr = [ clutter_xcorr_flip; clutter_xcorr(2:end) ];
    clutter_xcorr = ( clutter_xcorr - mean(ave_clutter_tmp)^2 ) / ...
        (mean(ave_clutter_tmp)+(mean(ave_clutter_tmp)==0))^2;
    clutter_xcorr(max_lag + 1) = 0;
    lh_clutter = plot((-max_lag:max_lag)*dt, squeeze(clutter_xcorr), '-b');
...
%         xcorr( ave_bkgrnd_tmp', [], max_lag, 'unbiased' );
    ave_bkgrnd_tmp = ...
        zeros(num_stim_steps,1);
    ave_bkgrnd_tmp(:,1) = ...
        full(ave_bkgrnd{layer,1}(stim_steps));
    [bkgrnd_xcorr, ...
     bkgrnd_xcorr_std, ...
     bkgrnd_xcorr_dist, ...
     bkgrnd_xcorr_mean, ...
     bkgrnd_xcorr_lags, ...
     bkgrnd_xcorr_figs] = ...
     pvp_xcorr( ave_bkgrnd_tmp, ...
            ave_bkgrnd_tmp, ...
            max_lag, ...
            1, [num_stim_steps, 1], ...
            1, [num_stim_steps, 1], ...
            1, 1 );        
    bkgrnd_xcorr = squeeze( bkgrnd_xcorr(1,1,:) );
    bkgrnd_xcorr_flip = ...
        flipdim( bkgrnd_xcorr, 1 );
    bkgrnd_xcorr = [ bkgrnd_xcorr_flip; bkgrnd_xcorr(2:end) ];
    bkgrnd_xcorr = ( bkgrnd_xcorr - mean(ave_bkgrnd_tmp)^2 ) / ...
        (mean(ave_bkgrnd_tmp)+(mean(ave_bkgrnd_tmp)==0))^2;
    bkgrnd_xcorr(max_lag + 1) = 0;
    lh_bkgrnd = plot((-max_lag:max_lag)*dt, squeeze(bkgrnd_xcorr), '-k');
...
%         xcorr( ave_target_tmp', ave_clutter_tmp', max_lag, 'unbiased' );
    [target2clutter_xcorr, ...
     target2clutter_xcorr_std, ...
     target2clutter_xcorr_dist, ...
     target2clutter_xcorr_mean, ...
     target2clutter_xcorr_lags, ...
     ...
     pvp_xcorr( ave_target_tmp, ...
            ...
            1, [num_stim_steps, 1], ...
            1, 1 );        
    target2clutter_xcorr = squeeze( target2clutter_xcorr(1,1,:) );
    target2clutter_xcorr_flip = ...
        flipdim( target2clutter_xcorr, 1 );
    target2clutter_xcorr = [ target2clutter_xcorr_flip; target2clutter_xcorr(2:end) ];
    target2clutter_xcorr = ...
    ...
    plot_title = ...
	  [layerID{layer}, ' Power(', int2str(layer), ')'];
    fig_tmp = figure('Name',plot_title);
    fig_list = [fig_list; fig_tmp];	
