...
if ($ARGV[0] && $ARGV[1] && $ARGV[2]) {
    $out = &extractAnnotations($ARGV[0], $ARGV[1], $ARGV[2]);
    print "extractAnnotations: Program complete.\n";
} else {
    &eiPostUsage();
}
#####
...
sub eiPostUsage () {
    die "\n\nUsage: ./extractAnnotations.pl WNID input_dir destination_dir\n\tWNID can be a text file, a folder path, or a single ID\n\n\n";
}
...
#Ask user if the program should get the child nodes
    print "\nextractAnnotations: Would you like to extract the children of the input? [y/n] ";
    my $userChoice = <STDIN>;
    chomp($userChoice);
    my $correctAnswer = 0;
    while ($correctAnswer == 0) {
        ...
        print "extractAnnotations: Please respond with 'y' or 'n': ";
        $userChoice = <STDIN>;
        chomp($userChoice);
    }
...
#Check to see if the user has input a WNID, a folder, or a text file listing WNIDs
    ...
    if ($inputType==1) { #The user has input a synset id
        ...
        $inputCategory =~ s/\/$//g;
        $inputCategory =~ s/\s/\\ /g;
        print "extractAnnotations: Getting list of files from the input dir $inputCategory\n";
        my $ext = 'tar';
        ...
        print "extractAnnotations: Found $numFilesFound files to extract!\n";
...
#Parse through input items, everything inside this loop is done for each input item
    ...
    foreach my $lineInput (@inArray) {
        ...
        if ($userChoice =~ /^y$/) { #Grab children
            ...
            print "\nextractAnnotations: Downloading list of child synsets...\n";
            ...
            foreach my $WNID (@synsets) {
                ...
                if ($result == 1) {
                    ...
                    print "extractAnnotations: Failed to extract $WNID.\n\n-------------------\n\n";
                }
            }
        } else {
           ...
           if ($result == 1) {
               print "extractAnnotations: Extracted $lineInput!\n\n-------------------\n\n";
           } else {
               print "extractAnnotations: Failed to extract $lineInput\n\n-------------------\n\n";
           }
        }
    }
    return 1;
...
## doExtract 
##
## Extracts $categoryWNID annotations from $ARCH_DIR to $destDir
##  Inputs:
...
#Create directories & extract files
    print "doExtract: Extracting annotations...\n\n";
    my $firstRun = 1; 
    my $firstDir = '';
    foreach my $dir (@outputDirs) {
        ...
        if ($firstRun) {
            ...
            system("mkdir $cleanDir") unless (-d "$cleanDir");
            system("tar -xkzf $ARCH_DIR/$archFile -C $dir/ 2> $TMP_DIR/tarout.err");
            #system("rm -f $destDir/synset\_info.txt");
            ...
            foreach my $line (@TARERR) {
                if ($line =~ /Already exists/) {
                    print "doExtract: The annotations have already been extracted for this tar file.\n";
                    return 0;
