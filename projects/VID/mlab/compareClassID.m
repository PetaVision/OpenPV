...
if strcmp(run_type, "VID")
  i_scale_list = [1 : num_Recon_frames_per_layer];
endif
...
for i_scale = i_scale_list 
  ...
  for i_frame = 1 : min(pred_num_frames, gt_num_frames) 
    ...
    i_Recon_ndx = find(pred_time == last_time{:});
    if ~isempty(i_Recon_ndx) %%any(pred_time == last_time{:})
	...
	drawnow
	%%get(th)
	%%Recon_nx = Recon_hdr{1,i_Recon_ndx}.nx;
	%%Recon_ny = Recon_hdr{1,i_Recon_ndx}.ny;
