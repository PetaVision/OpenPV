...
output_dir = "/nh/compneuro/Data/Depth/LCA/stack_v2_nnz/"; 
...
%%keyboard;
plot_SparsityProbe = 1;
if plot_SparsityProbe
  first_StatsProbe_line = 1; %%max([(last_StatsProbe_line - StatsProbe_plot_lines), 1]);
  StatsProbe_plot_lines = 70;
%%  StatsProbe_list = ...
%%      {["Error"],["_Stats.txt"]; ...
%%       ["V1"],["_Stats.txt"]};
  StatsProbe_list = ...
  ...
  for i_StatsProbe = 1 : num_StatsProbe_list
    ...
    for i_line = 1:num_lines
      ...
      StatsProbe_sparsity_str = StatsProbe_line(StatsProbe_sparsity_ndx1+10:StatsProbe_sparsity_ndx2-2);
      StatsProbe_sparsity_vals(i_line) = str2num(StatsProbe_sparsity_str);
      %% extract VThresh
      StatsProbe_vthresh_ndx1 = strfind(StatsProbe_line, "VThresh==");
      StatsProbe_vthresh_ndx2 = length(StatsProbe_line); 
      StatsProbe_vthresh_str = StatsProbe_line(StatsProbe_vthresh_ndx1+9:StatsProbe_vthresh_ndx2-1);
      StatsProbe_vthresh_vals(i_line) = str2num(StatsProbe_vthresh_str);
    endfor %%i_line
    fclose(StatsProbe_fid);
...
if analyze_nonSparse_flag
    ...
    nonSparse_norm_strength = [.0294 .0294 .17677 1 1 1];
    Sparse_std_ndx = [0 0 0 1 1 1];
    plot_flag = true;
...
if plot_weights0_2
  weights1_2_list = {};
   ...
   weights1_2_list = ...
       {...
          ["w50_"], ["V2S2ToV1S2Error"];...
          ["w51_"], ["V2S4ToV1S4Error"];...
          ["w52_"], ["V2S8ToV1S8Error"];...
          ["w50_"], ["V2S2ToV1S2Error"];...
          ["w51_"], ["V2S4ToV1S4Error"];...
          ["w52_"], ["V2S8ToV1S8Error"];...
       };
   ...
   weights0_1_list = ...
       {...
          ["w8_"], ["V1S2ToLeftError"];...
          ["w0_"], ["V1S4ToLeftError"];...
          ["w10_"], ["V1S8ToLeftError"];...
          ["w26_"], ["V1S2ToRightError"];...
          ["w27_"], ["V1S4ToRightError"];...
          ["w28_"], ["V1S8ToRightError"];...
       };
   image_list = ...
       {...
          ["a3_"], ["LeftRescale"];...
