...
no_clobber = false; %%true;
  ...
  run_type = "DCNN";
    output_dir = "/home/ec2-user/mountData/dictLearn/binocDCNN";
    checkpoint_parent = "/home/ec2-user/mountData/dictLearn/";
    checkpoint_children = {"binocDCNN"}; 
...
if analyze_Recon
  ...
  elseif strcmp(run_type, "DCNN")  
    ...
    ["a1_"],  ["RightImage"];
	 ["a4_"],  ["LeftImageDecon"];
	 ["a5_"],  ["RightImageDecon"];
	 ["a10_"],  ["LeftImageDeconS1"];
	 ["a11_"],  ["RightImageDeconS1"];
	 ["a15_"],  ["LeftImageDeconS2"];
	 ["a16_"],  ["RightImageDeconS2"];
	 ["a21_"],  ["LeftImageDeconS3"];
	 ["a22_"],  ["RightImageDeconS3"]};
    %% list of layers to unwhiten
    num_Recon_list = size(Recon_list,1);
    Recon_unwhiten_list = zeros(num_Recon_list,1);
    %% list of layers to use as a normalization reference for unwhitening
...
%%    nonSparse_norm_list = {[],[]; [],[]};
  ...
  elseif strcmp(run_type, "DCNN") 
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    nonSparse_list = ...
	{["a6_"], ["LeftImageDeconError"];
    ["a7_"], ["RightImageDeconError"]}; 
    num_nonSparse_list = size(nonSparse_list,1);
    nonSparse_skip = repmat(1, num_nonSparse_list, 1);
    nonSparse_norm_list = ...
        {["a0_"], ["LeftImage"];
  ...
  if strcmp(run_type, "Heli_DPTX3") 
   ...
   elseif strcmp(run_type, "dSCANN") || strcmp(run_type, "DCNN") 
    ...
    if ~checkpoint_weights_movie
      ...
      weights_list = ...
          {["S1ToLeftImageDeconError"], ["_W"]
           ["S1ToRightImageDeconError"], ["_W"]};
      labelWeights_list = {}; %%...
      checkpoints_list = getCheckpointList(checkpoint_parent, checkpoint_children);
    endif %% checkpoint_weights_movie
    num_checkpoints = size(checkpoints_list,1);
...
%%	  {["V2ToLabelError"], ["_W"]};
  ...
  for i_weights1_2 = 1 : num_weights1_2_list
	...
	for i_fig0_2 = 1 : num_figs0_2
	  figure(weights1_2_fig(i_fig0_2))
	  saveas(weights1_2_fig(i_fig0_2), [weights1_2_dir, filesep, weights1_2_name, "_", weights0_1_list{i_weights1_2, 1}, "_",     num2str(i_fig0_2), ".png"], "png");
	endfor
      endif
	...
	imwrite(uint8(weight_patch0_2_array_uint8), ...
		[weights1_2_movie_dir, filesep, weights1_2_name, "_", weights0_1_list{i_weights1_2, 2}, "_",  num2str(i_fig0_2), ".png"], "png");
...
%%	  weightsN_Nplus1_vals{i_weightN_Nplus1, i_layerN_Nplus1+1}(:, :, :, kf_preN_Nplus1_rank) = ...
	...
	if plot_flag && ...
	  ...
	  for i_figN_Nplus1 = 1 : num_figsN_Nplus1
	    figure(weightsN_Nplus1_fig(i_figN_Nplus1));
	    saveas(weightsNminus1_Nplus1_fig, [weightsN_Nplus1_dir, filesep, weightsN_Nplus1_str{i_figN_Nplus1}, "_", layersN_Nplus1_list{i_figN_Nplus1, 1}, ".png"], "png");
	    if ~isempty(labelWeights_vals) 
	...
	if plot_flag && ...
	  ...
	  saveas(weightsNminus1_Nplus1_hist_fig, ...
		 [weightsN_Nplus1_dir, filesep, "weightsN_Nplus1_hist_", ...
		  weightsN_Nplus1_str{i_figN_Nplus1}, "_", layersN_Nplus1_list{i_figN_Nplus1}, 1], "png");
	endif %% plotNminus1_Nplus1_flag
