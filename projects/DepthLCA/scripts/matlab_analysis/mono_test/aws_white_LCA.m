...
checkptDir = '/home/ec2-user/mountData/saved_data/white_LCA_dictLearn/'
...
figure;
subplot(1, 2, 1);
imagesc(leftVals(:, :, 1, minIdx));
subplot(1, 2, 2);
imagesc(rightVals(:, :, 1, minIdx));
title('Left');
colormap(gray);

figure;
subplot(1, 2, 1);
imagesc(leftVals(:, :, 1, maxIdx));
subplot(1, 2, 2);
imagesc(rightVals(:, :, 1, maxIdx));
title('Right');
colormap(gray);
