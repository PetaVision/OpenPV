...
outdir = '/home/ec2-user/mountData/dictLearn/depth_alex/';
timestamp = [outdir '/timestamps/DepthImage.txt'];
outPvpFile = [outdir 'a13_FinalOut.pvp'];
gtPvpFile = [outdir 'a9_DepthDownsample.pvp'];
...
for(i = 1:numFrames)
   ...
   outFilename = [scoreDir num2str(targetTime) '_gtVsEst_error_' num2str(d_err*100) '.png']
   outImg = disp_to_color([estData;gtData]);
   imwrite(outImg, outFilename);
   %handle = imshow(outImg);
