...
int main(int argc, char * argv[])
{
    ...
    {
        printf("Usage: %s filename\nThis function parses the header of filename assuming filename is a pvp file.\n", argv[0]);
        return status;
    }
    int arg;
    for (arg=1; arg<argc; arg++)
    {
        ...
        else
        {
            ...
            if (status1!=PARSE_SUCCESS) status=PARSE_FAILURE;
            fclose(fid);
        }
    }
...
int parseheader(FILE * fid)
{
    ...
    {
        fprintf(stderr, "Number of parameters (%d) inconsistent with headersize (%d)\n", params[INDEX_NUM_PARAMS], headersize);
    ...
    case PVP_KERNEL_FILE_TYPE:
        ...
        break;
    default:
        correct_numparams = !params[1];
        break;
    }
    if (correct_numparams != params[1])
    {
        fprintf(stderr, "Number of parameters (%d) inconsistent with file type (%d)\n",
                params[INDEX_NUM_PARAMS], params[INDEX_FILE_TYPE]);
