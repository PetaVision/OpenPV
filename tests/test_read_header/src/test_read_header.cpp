...
int main(int argc, char *argv[]) {
   ...
   if (time != write_time)
      status = -1;
   if (filetype != write_filetype)
      status = -1;
   if (datatype != write_datatype)
      status = -1;
   ...
   if (params[INDEX_NUM_PARAMS] != numParams)
      status = -INDEX_NUM_PARAMS;
   if (params[INDEX_FILE_TYPE] != filetype)
      status = -INDEX_FILE_TYPE;
   if (params[INDEX_NX] != loc.nx)
      status = -INDEX_NX;
   if (params[INDEX_NY] != loc.ny)
      status = -INDEX_NY;
   if (params[INDEX_NF] != loc.nf)
      status = -INDEX_NF;
   if (params[INDEX_NUM_RECORDS] != numRecords)
      status = -INDEX_NUM_RECORDS;
   if (params[INDEX_RECORD_SIZE] != localSize)
      status = -INDEX_RECORD_SIZE;

   if (params[INDEX_DATA_SIZE] != size_float)
      status = -INDEX_DATA_SIZE;

   if (params[INDEX_NX_PROCS] != nxProcs)
      status = -INDEX_NX_PROCS;
   if (params[INDEX_NY_PROCS] != nyProcs)
      status = -INDEX_NY_PROCS;

   if (params[INDEX_NX_GLOBAL] != nxProcs * loc.nx)
      status = -INDEX_NX_GLOBAL;
   if (params[INDEX_NY_GLOBAL] != nyProcs * loc.ny)
      status = -INDEX_NY_GLOBAL;

   if (params[INDEX_KX0] != 0)
      status = -INDEX_KX0;
   if (params[INDEX_KY0] != 0)
      status = -INDEX_KY0;

   // INDEX_NB is no longer used

   if (params[INDEX_NBANDS] != loc.nf)
      status = -INDEX_NBANDS;
   ;
