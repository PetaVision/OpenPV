      ...
      for (int b = 0; b < nbatch; ++b) {
         ...
         for (int r = 0; r < numRows; ++r) {
            for (int c = 0; c < numColumns; ++c) {
               ...
               sum += allPartialNorms.at(index);
            }
         }
         double correctValue = sum;
...
void initLogFile(std::shared_ptr<MPIBlock const> mpiBlock) {
   ...
   std::string logFileName("NormProbeAggregatorTest_8_");
...
std::shared_ptr<MPIBlock const> initMPIBlock() {
   ...
   if (commSize != 8) {
      ErrorLog().printf("NormProbeAggregatorTest must be called under MPI with 8 processes.\n");
   ...
   // This mimics the Communicator's LocalMPIBlock that NormProbeAggregator typically uses.
   return result;
}
