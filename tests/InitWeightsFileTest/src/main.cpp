...
int main(int argc, char *argv[]) {
   ...
   FatalIf(
         ...
         "Presynaptic numExtended %ld != number of data patches %ld\n",
         numExtended,
         numPatches);
   ...
   for (long index = 0; index < numExtended; index++) {
      ...
      long globalIndex = kIndex(xExtIndex, yExtIndex, fIndex, nxExtGlobal, nyExtGlobal, nf);
      ...
      for (int y = yStart; y < yStart + patch.ny; y++) {
         for (int x = xStart; x < xStart + patch.nx; x++) {
            for (int f = 0; f < nf; f++) {
               long k                   = kIndex(x, y, f, nxp, nyp, nfp);
               float w                  = weights[k];
               float indexObservedFloat = std::floor(w);
               float kObservedFloat     = (w - indexObservedFloat) * numItemsInPatch;
               long kObserved           = (long)std::nearbyint(kObservedFloat);
               if (kObserved != k or (long)indexObservedFloat != globalIndex) {
                  ErrorLog().printf(
                        "Rank %d, Patch %ld (global index %ld), (x,y,z)=(%d,%d,%d): "
