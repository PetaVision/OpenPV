...
int TriggerTestLayer::updateState(double time, double dt){
   ...
   pvInfo().printf("%s: time=%f, dt=%f, needUpdate=%d, triggerOffset=%f\n", name, time, dt, needUpdate(time, dt), triggerOffset);
   if(strcmp(name, "notrigger") == 0){
      pvErrorIf(!(needUpdate(time, dt) == true), "Test failed at %s. Expected true, found false.\n", getName());
   }
   //Trigger with offset of 0, assuming display period is 5
      ...
      if(((int)time-1) % 5 == 0){
         ...
         pvErrorIf(!(needUpdate(time, dt) == false), "Test failed at %s. Expected false, found true.\n", getName());
      }
   }
