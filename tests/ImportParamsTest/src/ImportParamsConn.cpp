...
ImportParamsConn::ImportParamsConn(std::shared_ptr<ParamsIO> paramsIO, Communicator const *comm) {
   initialize_base();
...
void ImportParamsConn::initialize(std::shared_ptr<ParamsIO> paramsIO, Communicator const *comm) {
   HyPerConn::initialize(paramsIO, comm);

   // Test grabbed array value
   std::vector<double> delayVals;
   paramsIO->ioParam(ParamsIOSwitch::Read, "delay", &delayVals);
