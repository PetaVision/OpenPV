...
SharedConnDebugInitWeights::SharedConnDebugInitWeights(std::shared_ptr<ParamsIO> paramsIO, Communicator const *comm)
      : HyPerConn() {
   SharedConnDebugInitWeights::initialize(paramsIO, comm);
}

SharedConnDebugInitWeights::~SharedConnDebugInitWeights() {}

void SharedConnDebugInitWeights::initialize(std::shared_ptr<ParamsIO> paramsIO, Communicator const *comm) {
   HyPerConn::initialize(paramsIO, comm);
}

SharedWeights *SharedConnDebugInitWeights::createSharedWeights() {
   return new SharedWeightsTrue(mParamsIO, mCommunicator);
}

int SharedConnDebugInitWeights::ioParamsFillGroup(ParamsIOSwitch ioSwitch) {
   int status = HyPerConn::ioParamsFillGroup(ioSwitch);
   ioParam_weightInitType(ioSwitch);
   return status;
}

void SharedConnDebugInitWeights::ioParam_weightInitType(ParamsIOSwitch ioSwitch) {
...
void SharedConnDebugInitWeights::smartWeights(float *dataStart, int k) {
   ...
   const int nfp   = patchSize->getPatchSizeF();
...
void SharedConnDebugInitWeights::initializeCocircWeights(float *dataStart, int numPatches) {
   ...
   float rotate     = mParamsIO->readValue<float>("rotate");
   ...
   // fraction of delta_radius_curvature
   float sigma_kurve = mParamsIO->readValue<float>("sigmaKurve");
...
void SharedConnDebugInitWeights::cocircCalcWeights(
   ...
   auto *patchSize   = getComponentByType<PatchSize>();
   const int nxPatch = patchSize->getPatchSizeX();
   const int nyPatch = patchSize->getPatchSizeY();
...
void SharedConnDebugInitWeights::initializeGaborWeights(float *dataStart, int numPatches) {
   ...
   float sigma = mParamsIO->readValue<float>("sigma");
   float rMax = mParamsIO->readValue<float>("rMax");
   float lambda = mParamsIO->readValue<float>("lambda");
   float strength = mParamsIO->readValue<float>("strength");
...
void SharedConnDebugInitWeights::gaborWeights(
   ...
   auto *patchSize = getComponentByType<PatchSize>();
   const int nx    = patchSize->getPatchSizeX(); //(int) wp->nx;
   const int ny    = patchSize->getPatchSizeY(); //(int) wp->ny;
   const int nf    = patchSize->getPatchSizeF();
...
int SharedConnDebugInitWeights::dataIndexToUnitCellIndex(int dataIndex, int *kx, int *ky, int *kf) {
   ...
   if (preWeights->getSharedWeightsFlag()) {
