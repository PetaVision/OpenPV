...
void SharedConnDebugInitWeights::initializeSmartWeights(float *dataStart, long numPatches) {
   auto *weightsPair    = getComponentByType<WeightsPair>();
   long overallPatchSize = weightsPair->getPreWeights()->getPatchSizeOverall();
...
void SharedConnDebugInitWeights::smartWeights(float *dataStart, int k) {
   ...
   const long sxp    = preWeights->getPatchStrideX();
   const long syp    = preWeights->getPatchStrideY();
   const long sfp    = preWeights->getPatchStrideF();
...
void SharedConnDebugInitWeights::initializeGaborWeights(float *dataStart, long numPatches) {
...
int SharedConnDebugInitWeights::dataIndexToUnitCellIndex(long dataIndex, int *kx, int *ky, int *kf) {
   auto *weightsPair         = getComponentByType<WeightsPair>();
   Weights *preWeights       = weightsPair->getPreWeights();
