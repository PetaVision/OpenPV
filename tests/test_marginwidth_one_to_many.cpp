...
int main(int argc, char * argv[]) {
    ...
    fprintf(stderr, "If test_marginwidth_one_to_many works correctly, there will be two margin width errors below.\n");
    hc = new HyPerCol("test_marginwidth_one_to_many column", argc, argv);
    Retina * retinaSmall = new Retina("test_marginwidth_one_to_many Retina small marginWidth", hc);
    layerA = new ANNLayer("test_marginwidth_one_to_many ANNLayer", hc);
    KernelConn * retinaSmall_layerA = new KernelConn("test_marginwidth_one_to_many Retina to ANNLayer", hc, retinaSmall, layerA, CHANNEL_EXC);
    ...
    hc = new HyPerCol("test_marginwidth_one_to_many column", argc, argv);
    Retina * retinaLarge = new Retina("test_marginwidth_one_to_many Retina large marginWidth", hc);
    layerA = new ANNLayer("test_marginwidth_one_to_many ANNLayer", hc);
    // need to redefine this layer since previous "delete hc;" statement deleted the layers.
    KernelConn * retinaLarge_layerA = new KernelConn("test_marginwidth_one_to_many Retina to ANNLayer", hc, retinaLarge, layerA, CHANNEL_EXC);
    ...
    hc = new HyPerCol("test_marginwidth_one_to_many column", argc, argv);
    Retina * retinaCorrect = new Retina("test_marginwidth_one_to_many Retina correct marginWidth", hc);
    layerA = new ANNLayer("test_marginwidth_one_to_many ANNLayer", hc);
    // need to redefine this layer since previous "delete hc;" statement deleted the layers.
    KernelConn * retinaCorrect_layerA = new KernelConn("test_marginwidth_one_to_many Retina to ANNLayer", hc, retinaCorrect, layerA, CHANNEL_EXC);
