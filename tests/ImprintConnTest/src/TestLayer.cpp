...
TestLayer::TestLayer(const char *name, HyPerCol *hc) { ANNLayer::initialize(name, hc); }
      ...
      for (int iX = loc->halo.lt; iX < nx + loc->halo.lt; iX++) {
         for (int iFeature = 0; iFeature < nf; iFeature++) {
            ...
            if (timef == 10) {
               ...
               // Order of imprinting: kernel 0 at ts 4, kernel 1 at ts 5, kernel 2 at ts 6, kernel
               // 3 at ts 7
