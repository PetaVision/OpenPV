...
int checkL2NormProbe(
      std::string const &probeNameBase,
      int batchIndex,
      std::shared_ptr<FileManager> fileManager);
int compareL2NormFiles(std::string const &correct, std::string const &observed);
int compareL2NormValues(
      L2NormProbeValues const &correct,
      L2NormProbeValues const &observed,
...
int run(PV_Init *pv_init) {
   ...
   delete hc; // We need to call the HyPerCol destructor before calling checkOutput(),
              // since that is when the L2NormProbe output files are certain to be flushed.
...
int checkProbe(
   ...
   for (int b = startElement; b < stopElement; ++b) {
      if (checkL2NormProbe(probeNameBase, b, fileManager) != PV_SUCCESS) {
         status = PV_FAILURE;
      }
   }
   return status;
...
// Compares two files with filename [probeNameBase]L2Norm_batchElement_[b].txt,
...
int checkL2NormProbe(
      ...
      int batchIndex,
      std::shared_ptr<FileManager> fileManager) {
   std::string filename = probeNameBase + ("L2Norm_batchElement_");
   filename.append(std::to_string(batchIndex)).append(".txt");

   std::string correctValuesPath("input/");
   correctValuesPath.append(filename);

   std::shared_ptr<FileStream> observedL2NormStream = FileStreamBuilder(
                                                            fileManager,
                                                            filename,
                                                            true /*isText*/,
                                                            true /*readOnlyFlag*/,
                                                            ...
                                                            false /*verifyWrites*/)
                                                            .get();
   std::string observedValuesPath = observedL2NormStream->getFileName();
   observedL2NormStream->close();
   observedL2NormStream = nullptr;

   int status = compareL2NormFiles(correctValuesPath, observedValuesPath);
   return status;
}

int compareL2NormFiles(std::string const &correctPath, std::string const &observedPath) {
   FILE *correctfp  = fopen(correctPath.c_str(), "r");
   FILE *observedfp = fopen(observedPath.c_str(), "r");
   FatalIf(!correctfp, "compareL2NormFiles unable to open %s for reading.\n", correctPath.c_str());
   FatalIf(
         !observedfp, "compareL2NormFiles unable to open %s for reading.\n", observedPath.c_str());

   L2NormProbeValues correctValues, observedValues;
   int status     = PV_SUCCESS;
   int linenumber = 0;
   while (true) {
      ++linenumber;
      ...
      if (numReadCorrect == 4 && numReadObserved == 4) {
         int status1 = compareL2NormValues(correctValues, observedValues, observedPath, linenumber);
         if (status1 != PV_SUCCESS) {
            status = PV_FAILURE;
         }
         continue;
