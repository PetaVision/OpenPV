...
L2NormProbeLocal makeProbeLocal(char const *name, HyPerCol &hc, BaseLayer *targetLayer);
...
double computeCorrectValue(int batchIndex, BaseLayer *targetLayer) {
   ...
   for (long k = 0; k < numNeurons; ++k) {
      ...
      double v = (double)targetData[kExt];
      sum += v * v;
   }
   return sum;
}
   ...
   if (maskNf == targetNf) {
      ...
      for (int k = 0; k < maskNumNeurons; ++k) {
         ...
         if (maskData[kExtMask]) {
            ...
            if (maskData[kExtMask]) {
               double v = (double)targetData[kExtTarget];
               sum += v * v;
            }
         }
      }
...
int run(PV_Init *pv_init_obj) {
   FatalIf(
         ...
         "L2NormProbeLocalTest should be run without a params file.\n");
...
int runNoMask(PV_Init *pv_init_obj) {
   ...
   L2NormProbeLocal probeLocal = makeProbeLocal("ProbeWithNoMask", hc, targetLayer);
...
int runWithMask(PV_Init *pv_init_obj) {
   ...
   L2NormProbeLocal probeLocal = makeProbeLocal("ProbeWith3FeatureMask", hc, targetLayer);
   std::vector<std::vector<double>> correctValues(3);
...
int runWithSingleFeatureMask(PV_Init *pv_init_obj) {
   ...
   L2NormProbeLocal probeLocal = makeProbeLocal("ProbeWith1FeatureMask", hc, targetLayer);
   std::vector<std::vector<double>> correctValues(3);
