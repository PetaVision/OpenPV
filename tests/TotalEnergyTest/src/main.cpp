...
int main(int argc, char * argv[]) {
   ...
   if (generateFlag && (testrunFlag||testcheckpointFlag||testioparamsFlag)) {
      if (rank==0) {
         pvErrorNoExit().printf("%s error: --generate option conflicts with the --test* options.\n", argv[0]);
      }
   ...
   if (!(generateFlag||testrunFlag||testcheckpointFlag||testioparamsFlag)) {
      if (rank==0) {
         pvErrorNoExit().printf("%s error: At least one of \"--generate\", \"--testrun\", \"--testcheckpoint\", \"--testioparams\" must be selected.\n", argv[0]);
      }
      MPI_Barrier(MPI_COMM_WORLD); // Make sure no child processes take down the MPI environment before root process prints error message.
      exit(EXIT_FAILURE);
   }
...
int testcheckpoint(PV_Init * initObj, int rank) {
   ...
   if (!hasrestart) {
      if (rank==0) {
         pvErrorNoExit().printf("%s error: --testcheckpoint requires either the -r or the -c option.\n", initObj->getProgramName());
      }
