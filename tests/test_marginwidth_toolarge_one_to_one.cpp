/*
 * test_marginwidth_correctsize_one_to_one.cpp
 * Tests HyPerCol::checkPatchSize() (a private method called by HyPerCol::run(int) )
 * on a one-to-one connection where the margin width is larger than necessary.
...
int main(int argc, char * argv[])
{
   ...
   cl_args[1] = strdup("-p");
   cl_args[2] = strdup("input/test_marginwidth_toolarge_one_to_one.pv");

   hc = new HyPerCol("test_marginwidth_one_to_one column", 3, cl_args);
