...
outdir = '/nh/compneuro/Data/Depth/LCA/benchmark/stereo_train_slp_class_error/';
timestamp = [outdir '/timestamps/DepthImage.txt'];
outPvpFile = [outdir 'a8_Est.pvp'];
...
allScores = zeros(1, numFrames);
allTimes = zeros(1, numFrames);
...
for(i = 1:numFrames)
   ...
   allScores(i) = d_err;
   allTimes(i) = targetTime;
   %saveas(handle, outFilename);
end

figure;
handle = plot(allTimes, allScores);
title(sprintf('Score vs Time'));
scoreVsTimeFilename = [scoreDir, 'scoreVsTime.png']
saveas(handle, scoreVsTimeFilename);
