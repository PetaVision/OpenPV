...
int CliqueLayer::recvSynapticInput(HyPerConn * conn, PVLayerCube * activity, int neighbor)
{
   recvsyn_timer->start();
   ...
   unsigned int * marginIndices = (unsigned int *) calloc(numMargin, sizeof(int));
   assert(marginIndices != NULL);
   // get North margin indices
   for (unsigned int kPreExt = 0; kPreExt < nfExt * nxExt * marginUp; kPreExt++){
   ...
   assert(kMargin == nfExt * nxExt * marginUp + nfExt * marginLt * ny + nfExt * marginUp * ny);
   ...
   unsigned int patchSize = conn->nfp * conn->nxp * conn->nyp;
   ...
   unsigned int * patchActiveIndices = calloc(patchSize, sizeof(unsigned int));
   for (int kPreNZ = 0; kPreNZ < numActiveExt; kPreNZ++) {
      ...
      for(int kyPatchExt = kyPreExt - nyPostPatch; kyPatchExt < kyPreExt + nyPostPatch; kyPatchExt++){
         for(int kxPatchExt = kxPreExt - nxPostPatch; kxPatchExt < kxPreExt + nxPostPatch; kxPatchExt++){
            for(int kfPatchExt = kfPreExt - nxPostPatch; kfPatchExt < kfPreExt + nfPostPatch; kfPatchExt++){
               unsigned int kPrePatchExt = kIndex(kxPatchExt, kyPatchExt, kfPatchExt, nxExt, nyExt, nfExt);
               if aPre[kPrePatchExt] == 0) continue;
               patchActiveIndices[numPatchActive++] = kPrePatchExt;
            }
         }
      }
      ...
      for(unsigned int kClique = 0; kClique < numActiveCliques; kClique++){
         ...
         // decompose kClique to compute product of clique activities
         pvdata_t cliqueProd = aPre[kPreExt];
         unsigned int kResidue = kClique;
         for(unsigned int iProd = 0; iProd < cliqueSize-1; iProd++){
            unsigned int kPatchActive = (unsigned int) (kResidue / (cliqueSize-1));
            unsigned int kPrePatchExt = patchActiveIndices[kPatchActive];
            cliqueProd *= aPre[kPrePatchExt];
         }
