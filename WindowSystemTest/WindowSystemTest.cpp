...
void * customgroup(const char * keyword, const char * name, HyPerCol * hc) {
   ...
   if (strcmp(keyword, "WindowProbe") == 0){
      addedGroup = new WindowProbe(name, hc);
   }
   int status = checknewobject((void *) addedGroup, keyword, name, hc); // checknewobject tests addedObject against null, and either prints error message to stderr or success message to stdout.
