...
% transducer.
%
% Note, the transducer inputs can only be used in 3D simulations and thus
% these examples are inherently memory and CPU intensive. Whilst the grid
% sizes and source frequencies used in the examples have been scaled down
% for the purpose of demonstrating the capabilities of the toolbox (the
% inputs do not necessarily represent realistic ultrasound settings), they
% still require a comparatively large amount of computational resources. To
% reduce this load it is advised to run the simulations in single precision
% by setting the optional input 'DataCast' to 'single'. Similarly, if you
% have access to a recent model GPU and the Accelereyes Jacket Toolbox, the
% simulation times can be significantly reduced by setting 'DataCast' to
% 'gsingle' (a modern GPU can reduce simulation times by more than an order
% of magnitude). The creation of a kWaveTransducer object will only work in
% versions of MATLAB recent enough to support user defined classes. 
%
...
[f_input, as_input] = spectrum([input_signal, zeros(1, 2*length(input_signal))], 1/kgrid.dt);
[f, as_1] = spectrum(sensor_data(1, :), 1/kgrid.dt);
[f, as_2] = spectrum(sensor_data(2, :), 1/kgrid.dt);
[f, as_3] = spectrum(sensor_data(3, :), 1/kgrid.dt);
