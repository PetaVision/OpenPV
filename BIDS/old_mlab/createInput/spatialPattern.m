...
m1 = [(0:floor(DIM(1)/2)) -(ceil(DIM(1)/2)-1:-1:1)]/DIM(1);
m2 = [(0:floor(DIM(2)/2)) -(ceil(DIM(2)/2)-1:-1:1)]/DIM(2);
m3 = [(0:floor(DIM(3)/2)) -(ceil(DIM(3)/2)-1:-1:1)]/DIM(3);

[X, Y, T] = meshgrid(m1, m2, m3);
% Generate the power spectrum
S_f = (X.^2 + Y.^2 + T.^2).^(BETA/2);
...
x = ifftn(S_f .* (cos(2*pi*phi)+i*sin(2*pi*phi)));
