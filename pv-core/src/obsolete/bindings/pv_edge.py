...
RetinaToV1SimpleInhibSynapseParams = (
	1.0, #G_usePBC: periodic boundary conditions
	2*2+2*2,#G_R2: neighborhood radius
	SIGMA_EDGE, #G_SIGMA (not squared)
	ASPECT_RATIO, #G_ASPECT (1/aspect ratio)
	5.0 * GLOBAL_GAIN #GAUSS_WEIGHT_SCALE
)

V1SimpleInhibToV1SimpleLeftFlankSynapseParams = (
	1.0, #G_usePBC: periodic boundary conditions
	2*2+2*2,#G_R2: neighborhood radius
	SIGMA_EDGE, #G_SIGMA (not squared)
	ASPECT_RATIO, #G_ASPECT (1/aspect ratio)
...
V1SimpleInhibToV1SimpleRightFlankSynapseParams = (
	1.0, #G_usePBC: periodic boundary conditions
	2*2+2*2,#G_R2: neighborhood radius
	SIGMA_EDGE, #G_SIGMA (not squared)
	ASPECT_RATIO, #G_ASPECT (1/aspect ratio)
...
pypv.command(PV_ACTION_SET_LAYER_PARAMS, (LAYER_FLANKING, PV_HANDLER_LIF2),
				   (V_REST, V_EXC, V_INH, V_INHB, 		
				   TAU_VMEM, TAU_EXC, TAU_INH, TAU_INHB, 
				   VTH_REST-2.5,  TAU_VTH, DELTA_VTH,
				   0.25, NOISE_AMP*( 1.0/TAU_EXC ) * ( ( TAU_INH * (V_REST-V_INH) + TAU_INHB * (V_REST-V_INHB) ) / (V_EXC-V_REST) ), 
				   0.25, NOISE_AMP*1.0, 
				   0.25, NOISE_AMP*1.0))
...
#pypv.command(PV_ACTION_ADD_CONNECTION, (LAYER_V1, LAYER_V1, 0, 0, PV_HANDLER_COCIRC1D),
    			#(1.0, # CCK_usePBC
				#2 * R2_COCIRC * R2_COCIRC, #EXCITE_R2
				#2 * SIGMA_DIST_COCIRC * SIGMA_DIST_COCIRC, # COCIRC_SIGMA_DIST2
				#DK * DK, # COCIRC_SIGMA_KURVE2
				#DTH * DTH, # COCIRC_SIGMA_COCIRC2
				#0.0, # COCIRC_SELF
				#10.0 * GLOBAL_GAIN)) # COCIRC_WEIGHT_SCALE

pypv.command(PV_ACTION_ADD_CONNECTION, (LAYER_R, LAYER_FLANKING, 0, 0, PV_HANDLER_GAUSS2D),
				RetinaToV1SimpleInhibSynapseParams)

# Flanking Inhibition
pypv.command(PV_ACTION_ADD_CONNECTION, (LAYER_FLANKING, LAYER_V1, 0, 1, PV_HANDLER_GAUSS2DX),
				V1SimpleInhibToV1SimpleLeftFlankSynapseParams)
pypv.command(PV_ACTION_ADD_CONNECTION, (LAYER_FLANKING, LAYER_V1, 0, 1, PV_HANDLER_GAUSS2DX),
				V1SimpleInhibToV1SimpleRightFlankSynapseParams)
...
inSpike = [ 0.0 for x in range(0,NX*NY*1*1)]
inSpike[(NY/2 - 2)*NX*1*1 + (NX/2 + 0)*1*1 + 0*1 + 0] = 1.0
inSpike[(NY/2 - 1)*NX*1*1 + (NX/2 + 0)*1*1 + 0*1 + 0] = 1.0
inSpike[(NY/2 + 0)*NX*1*1 + (NX/2 + 0)*1*1 + 0*1 + 0] = 1.0
inSpike[(NY/2 + 1)*NX*1*1 + (NX/2 + 0)*1*1 + 0*1 + 0] = 1.0
inSpike[(NY/2 + 2)*NX*1*1 + (NX/2 + 0)*1*1 + 0*1 + 0] = 1.0
pypv.command(PV_ACTION_INJECT, (LAYER_R, PV_BUFFER_V, 0), inSpike)   

pypv.command(PV_ACTION_RUN, 3, 0)   
