...
BasePVObject * PV_Init::create(char const * keyword, char const * name, HyPerCol * hc) const {
   BasePVObject * pvObject = factory->create(keyword, name, hc);
   if (pvObject == NULL) {
      if (getWorldRank()==0) {
         fflush(stdout);
...
HyPerCol * PV_Init::build() {
   ...
   for (int k=0; k<numGroups; k++) {
      ...
      if (!strcmp(kw, "HyPerCol")) {
         ...
         BasePVObject * addedObject = factory->create(kw, name, hc);
         if (addedObject==NULL) {
            if (hc->globalRank()==0) {
               fflush(stdout);
...
void PV_Init::printBuildAndRunDeprecationWarning(char const * functionName, char const * functionSignature) {
   if (buildandrunDeprecationWarning) {
      if (getWorldRank()==0) {
         fprintf(stderr, "\nWarning: %s(%s) has been deprecated.  Use the PV_Factory version of %s instead.\n\n",
