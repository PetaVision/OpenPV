...
PV_Factory::PV_Factory(PV_Factory const& orig) {
   copyKeywordHandlerList(orig.keywordHandlerList);
}

PV_Factory& PV_Factory::operator=(PV_Factory const& orig) {
   clearKeywordHandlerList();
   copyKeywordHandlerList(orig.keywordHandlerList);
}


int PV_Factory::registerCoreKeywords() {
   keywordHandlerList = std::vector<PV_KeywordHandler*>();
   ...
   registerKeyword("normalizeGroup", PV_Factory::createNull);
   registerKeyword("none", PV_Factory::createNull);
   registerKeyword("", PV_Factory::createNull);

   return PV_SUCCESS;
}

int PV_Factory::copyKeywordHandlerList(std::vector<PV_KeywordHandler*> const& orig) {
   for (std::vector<PV_KeywordHandler*>::const_iterator iter = orig.begin(); iter < orig.end(); iter++) {
      PV_KeywordHandler* kh = *iter;
      registerKeyword(kh->getKeyword(), kh->getCreator());
   }
   return PV_SUCCESS;
}

int PV_Factory::registerKeyword(char const * keyword, ObjectCreateFn creator) {
   PV_KeywordHandler const * keywordHandler = getKeywordHandler(keyword);
   if (keywordHandler != NULL) {
      return PV_FAILURE;
   }
   PV_KeywordHandler * newKeyword = new PV_KeywordHandler(keyword, creator);
   keywordHandlerList.push_back(newKeyword);
   return PV_SUCCESS;
}

BasePVObject * PV_Factory::create(char const * keyword, char const * name, HyPerCol * hc) const {
   PV_KeywordHandler const * keywordHandler = getKeywordHandler(keyword);
   return keywordHandler ? keywordHandler->create(name, hc) : NULL;
}

PV_KeywordHandler const * PV_Factory::getKeywordHandler(char const * keyword) const {
   for (std::vector<PV_KeywordHandler*>::const_iterator iter = keywordHandlerList.begin(); iter < keywordHandlerList.end(); iter++) {
      PV_KeywordHandler * typeCreator = *iter;
...
int PV_Factory::clearKeywordHandlerList() {
   for (std::vector<PV_KeywordHandler*>::const_iterator iter = keywordHandlerList.begin(); iter < keywordHandlerList.end(); iter++) {
      delete *iter;
   }
