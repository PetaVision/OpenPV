...
workspace_path = "/home/ec2-user/workspaceDataParallel/OpenPV/";
output_dir = "/home/ec2-user/mountData/batchLCA/s1TrainBatchOutput/";
...
if analyze_Sparse_flag
    ...
    outSparseDir = [output_dir, '/Sparse/'];
    mkdir(outSparseDir);


    numList = size(Sparse_list, 1);
    for(i_sparse = 1:numList)
       filename = [output_dir, Sparse_list{i_sparse, 1}, Sparse_list{i_sparse, 2}, '.pvp']
       [data, hdr] = readpvpfile(filename);
       assert(mod(hdr.nbands, hdr.nbatch) == 0);
       nframes = hdr.nbands/hdr.nbatch;

       nnzData = zeros(hdr.nbatch, nframes);
       times = zeros(hdr.nbatch, nframes);
       countData = zeros(hdr.nf, 1);
       for(i_frame = 1:nframes)
          for(i_batch = 1:hdr.nbatch)
             dataIdx = (i_frame - 1) * hdr.nbatch + i_batch;
             activeIdx = data{dataIdx}.values(:, 1);
             nnzData(i_batch, i_frame) = double(length(activeIdx))/(hdr.nx*hdr.ny*hdr.nf);
             times(i_batch, i_frame) = data{dataIdx}.time;
             %Convert activeIdx to features, activeIdx here already 0 idx, +1 to make 1 idx
             activeFeatureIdx = mod(activeIdx, hdr.nf) + 1;
             [tmpCount, ~] = hist(activeFeatureIdx, 1:hdr.nf);
             countData += tmpCount';
          end
       end
       [sortedCountData, rankedIdx] = sort(countData, 'descend');
       Sparse_hist_rank_array{i_sparse} = rankedIdx;

       %Make plot of sparsity, 1 per batch
       for(i_batch = 1:hdr.nbatch)
          outFilename = [outSparseDir, '/nnz_', Sparse_list{i_sparse, 2}, '_batch', num2str(i_batch), '_time', num2str(times(i_batch, end)), '.png'];
          h = figure;
          plot(times(i_batch, :), nnzData(i_batch, :));
          saveas(h, outFilename);
       end

       outFilename = [outSparseDir, '/hist_', Sparse_list{i_sparse, 2}, '_time', num2str(times(1, end)), '.png'];
       %Make plot of histogram of features
       h = figure;
       bar(1:hdr.nf, sortedCountData);
       saveas(h, outFilename);
    end
...
if analyze_nonSparse_flag
  ...
  if ~exist("nonSparse_norm_strength")
    ...
    outNonSparseDir = [output_dir, '/nonSparse/'];
    mkdir(outNonSparseDir);


    numList = size(Sparse_list, 1);
    for(i_sparse = 1:numList)
       filename = [output_dir, nonSparse_list{i_sparse, 1}, nonSparse_list{i_sparse, 2}, '.pvp']
       [errData, errHdr] = readpvpfile(filename);
       normFilename = [output_dir, nonSparse_norm_list{i_sparse, 1}, nonSparse_norm_list{i_sparse, 2}, '.pvp']
       [normData, normHdr] = readpvpfile(normFilename);

       assert(mod(errHdr.nbands, errHdr.nbatch) == 0);
       assert(mod(normHdr.nbands, normHdr.nbatch) == 0);
       nframes = hdr.nbands/hdr.nbatch;

       nonSparseData = zeros(hdr.nbatch, nframes);
       times = zeros(hdr.nbatch, nframes);
       countData = zeros(hdr.nf, 1);
       for(i_frame = 1:nframes)
          for(i_batch = 1:hdr.nbatch)
             dataIdx = (i_frame - 1) * hdr.nbatch + i_batch;
             %Error is defined as standard deviation of err over std of input
             assert(errData{dataIdx}.time == normData{dataIdx}.time);
             times(i_batch, i_frame) = errData{dataIdx}.time;
             errStd = std(errData{dataIdx}.values(:));
             inStd = std(normData{dataIdx}.values(:) * nonSparse_norm_strength(i_sparse));
             nonSparseData(i_batch, i_frame) = double(errStd)/inStd;
          end
       end

       %Make plot of error, 1 per batch
       for(i_batch = 1:hdr.nbatch)
          outFilename = [outNonSparseDir, '/nonSparse_', Sparse_list{i_sparse, 2}, '_batch', num2str(i_batch), '_time', num2str(times(i_batch, end)), '.png'];
          h = figure;
          plot(times(i_batch, :), nonSparseData(i_batch, :));
          saveas(h, outFilename);
       end
    end
