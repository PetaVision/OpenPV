...
%%setenv("GNUTERM","X11")
%%setenv("GNUTERM","aqua")
if exist("/nh/compneuro/Data/liblinear-multicore-2.1-2/matlab")
  addpath("/nh/compneuro/Data/liblinear-multicore-2.1-2/matlab");
  mlab_dir = "/nh/compneuro/Data/openpv/pv-core/mlab";
elseif exist("/shared/liblinear-2.1/matlab")
...
%% exclusive_flag: determines whether target categories are exclusive (exclusive_flag=1), non-exclusive (exclusive_flag=0), or partially exclusive wrt background (exclusive_flag=2)
...
if ~load_svm_flag
  ...
  training_max2X2_pool_matrix_array  = cell(num_Sparse_list + (num_Sparse_list>1),1);
  training_mean2X2_pool_matrix_array = cell(num_Sparse_list + (num_Sparse_list>1),1);
  training_max4X4_pool_matrix_array  = cell(num_Sparse_list + (num_Sparse_list>1),1);
  training_mean4X4_pool_matrix_array = cell(num_Sparse_list + (num_Sparse_list>1),1);
  ...
  training_max2X2_pool_matrix_array_long  = cell(num_Sparse_list + (num_Sparse_list>1),1);
  training_mean2X2_pool_matrix_array_long = cell(num_Sparse_list + (num_Sparse_list>1),1);
  training_max4X4_pool_matrix_array_long  = cell(num_Sparse_list + (num_Sparse_list>1),1);
  training_mean4X4_pool_matrix_array_long = cell(num_Sparse_list + (num_Sparse_list>1),1);
  ...
  training_max2X2_pool_matrix_array_predict  = cell(num_Sparse_list + (num_Sparse_list>1),1);
  ...
  xval_model_max2X2_pool_array  = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  xval_model_mean2X2_pool_array = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  xval_model_max4X4_pool_array  = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  xval_model_mean4X4_pool_array = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  ...
  model_max2X2_pool_array  = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  model_mean2X2_pool_array = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  model_max4X4_pool_array  = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  model_mean4X4_pool_array = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  ...
  predicted_label_max2X2_pool_array  = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  predicted_label_mean2X2_pool_array = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  predicted_label_max4X4_pool_array  = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  predicted_label_mean4X4_pool_array = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  ...
  accuracy_max2X2_pool_array  = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  accuracy_mean2X2_pool_array = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  accuracy_max4X4_pool_array  = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  accuracy_mean4X4_pool_array = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  ...
  prob_values_max2X2_pool_array  = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  prob_values_mean2X2_pool_array = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  prob_values_max4X4_pool_array  = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  prob_values_mean4X4_pool_array = cell(num_Sparse_list + (num_Sparse_list > 1), num_target_classes+(exclusive_flag==1));
  ...
  for i_Sparse = 1 : (num_Sparse_list + (num_Sparse_list > 1))
	...
	training_mean_pool_matrix = zeros(nf_Sparse_array(i_Sparse), ny_GT, nx_GT, num_GT_images);
      ...
      if max2X2_pool_flag
	training_max2X2_pool_matrix = zeros(2, 2, nf_Sparse_array(i_Sparse), ny_GT, nx_GT, num_GT_images);
      endif
      if mean2X2_pool_flag
	training_mean2X2_pool_matrix = zeros(2, 2, nf_Sparse_array(i_Sparse), ny_GT, nx_GT, num_GT_images);
      endif
      if max4X4_pool_flag
	training_max4X4_pool_matrix = (zeros(4, 4, nf_Sparse_array(i_Sparse), ny_GT, nx_GT, num_GT_images));
      endif
      if mean4X4_pool_flag
	training_mean4X4_pool_matrix = (zeros(4, 4, nf_Sparse_array(i_Sparse), ny_GT, nx_GT, num_GT_images));
      endif
	...
	if mean2X2_pool_flag
	  training_mean2X2_pool_matrix(:, :, :, :, :, j_Sparse_frame) = ...
	  Sparse_mean2X2_pool_array{i_Sparse}{i_Sparse_frame};
	endif
	if max4X4_pool_flag
	  training_max4X4_pool_matrix(:, :, :, :, :, j_Sparse_frame) = ...
	  Sparse_max4X4_pool_array{i_Sparse}{i_Sparse_frame};
	endif
	if mean4X4_pool_flag
	  training_mean4X4_pool_matrix(:, :, :, :, :, j_Sparse_frame) = ...
	  Sparse_mean4X4_pool_array{i_Sparse}{i_Sparse_frame};
	endif
	...
	if exclusive_flag == 1
	  ...
	  sparse(reshape(training_max2X2_pool_matrix, ...
			 [nf_Sparse_array(i_Sparse) * 2 * ny_GT * 2 * nx_GT, num_GT_images]));
	else
	  training_max2X2_pool_matrix = ...
	  sparse(reshape(training_max2X2_pool_matrix, ...
			 [nf_Sparse_array(i_Sparse) * 2 * 2, ny_GT * nx_GT * num_GT_images]));
	endif
      endif
      if mean2X2_pool_flag
	if exclusive_flag == 1
	  training_mean2X2_pool_matrix = ...
	  sparse(reshape(training_mean2X2_pool_matrix, ...
			 [nf_Sparse_array(i_Sparse) * 2 * ny_GT * 2 * nx_GT, num_GT_images]));
	else
	  training_mean2X2_pool_matrix = ...
	  sparse(reshape(training_mean2X2_pool_matrix, ...
	...
	if exclusive_flag == 1
	  ...
	  sparse(reshape(training_max4X4_pool_matrix, ...
			 [nf_Sparse_array(i_Sparse) * 4 * ny_GT * 4 * nx_GT, num_GT_images]));
	else
	  training_max4X4_pool_matrix = ...
	  sparse(reshape(training_max4X4_pool_matrix, ...
			 [nf_Sparse_array(i_Sparse) * 4 * 4, ny_GT * nx_GT * num_GT_images]));
	endif
      endif
      if mean4X4_pool_flag
	if exclusive_flag == 1
	  training_mean4X4_pool_matrix = ...
	  sparse(reshape(training_mean4X4_pool_matrix, ...
			 [nf_Sparse_array(i_Sparse) * 4 * ny_GT * 4 * nx_GT, num_GT_images]));
	else
	  training_mean4X4_pool_matrix = ...
	  sparse(reshape(training_mean4X4_pool_matrix, ...
	...
	if i_Sparse == 1
	  ...
	  if max2X2_pool_flag
	    training_max2X2_pool_matrix_array{num_Sparse_list+1, num_target_classes+1} = ...
	    training_max2X2_pool_matrix;
	  endif
	  if mean2X2_pool_flag
	    training_mean2X2_pool_matrix_array{num_Sparse_list+1, num_target_classes+1} = ...
	    training_mean2X2_pool_matrix;
	  endif
	  if max4X4_pool_flag
	    training_max4X4_pool_matrix_array{num_Sparse_list+1, num_target_classes+1} = ...
	    training_max4X4_pool_matrix;
	  endif
	  if mean4X4_pool_flag
	    training_mean4X4_pool_matrix_array{num_Sparse_list+1, num_target_classes+1} = ...
	    training_mean4X4_pool_matrix;
	  endif
	...
	elseif i_Sparse == num_Sparse_list+1
      ...
      if hist_pool_flag
	disp(["cross-validation for hist pool", " using ", Sparse_list{i_Sparse,2}])
	...
	train(training_label_vector{num_target_classes+1}, ...
      ...
      if max_pool_flag
	disp(["cross-validation for max pool", " using ", Sparse_list{i_Sparse,2}])
	...
	train(training_label_vector{num_target_classes+1}, ...
      ...
      if mean_pool_flag
	disp(["cross-validation for mean pool", " using ", Sparse_list{i_Sparse,2}])
	...
	train(training_label_vector{num_target_classes+1}, ...
      ...
      if max2X2_pool_flag
	disp(["cross-validation for max2X2 pool", " using ", Sparse_list{i_Sparse,2}])
	xval_model_max2X2_pool_array{i_Sparse, num_target_classes+1} = ...
	train(training_label_vector{num_target_classes+1}, ...
	      training_max2X2_pool_matrix, ...
	      liblinear_xval_options_str, 'col');
      endif
      if mean2X2_pool_flag
	disp(["cross-validation for mean2X2 pool", " using ", Sparse_list{i_Sparse,2}])
	xval_model_mean2X2_pool_array{i_Sparse, num_target_classes+1} = ...
	train(training_label_vector{num_target_classes+1}, ...
	      training_mean2X2_pool_matrix, ...
	      liblinear_xval_options_str, 'col');
      endif
      if max4X4_pool_flag
	disp(["cross-validation for max4X4 pool", " using ", Sparse_list{i_Sparse,2}])
	xval_model_max4X4_pool_array{i_Sparse, num_target_classes+1} = ...
	train(training_label_vector{num_target_classes+1}, ...
	      training_max4X4_pool_matrix, ...
	      liblinear_xval_options_str, 'col');
      endif
      if mean4X4_pool_flag
	disp(["cross-validation for mean4X4 pool", " using ", Sparse_list{i_Sparse,2}])
	xval_model_mean4X4_pool_array{i_Sparse, num_target_classes+1} = ...
	train(training_label_vector{num_target_classes+1}, ...
	      training_mean4X4_pool_matrix, ...
	      liblinear_xval_options_str, 'col');
      endif
      ...
      if hist_pool_flag
	disp(["train hist pool model", " using ", Sparse_list{i_Sparse,2}])
	...
	train(training_label_vector{num_target_classes+1}, ...
      ...
      if hist_pool_flag
	disp(["apply hist pool model", " using ", Sparse_list{i_Sparse,2}])
	...
	predict(training_label_vector{num_target_classes+1}, ...
      ...
      if max_pool_flag
	disp(["apply max pool model", " using ", Sparse_list{i_Sparse,2}])
	...
	predict(training_label_vector{num_target_classes+1}, ...
      ...
      if mean_pool_flag
	disp(["apply mean pool model", " using ", Sparse_list{i_Sparse,2}])
	...
	predict(training_label_vector{num_target_classes+1}, ...
      ...
      if max2X2_pool_flag
	disp(["apply max2X2 pool model", " using ", Sparse_list{i_Sparse,2}])
	[predicted_label_max2X2_pool_array{i_Sparse, num_target_classes+1}, ...
	 accuracy_max2X2_pool_array{i_Sparse, num_target_classes+1}, ...
	 prob_values_max2X2_pool_array{i_Sparse, num_target_classes+1}] = ...
	predict(training_label_vector{num_target_classes+1}, ...
		training_max2X2_pool_matrix, ...
		model_max2X2_pool_array{i_Sparse, num_target_classes+1}, ...
		['-b 1'], 'col');
      ...
      if mean2X2_pool_flag
	disp(["apply mean2X2 pool model", " using ", Sparse_list{i_Sparse,2}])
	[predicted_label_mean2X2_pool_array{i_Sparse, num_target_classes+1}, ...
	 accuracy_mean2X2_pool_array{i_Sparse, num_target_classes+1}, ...
	 prob_values_mean2X2_pool_array{i_Sparse, num_target_classes+1}] = ...
	predict(training_label_vector{num_target_classes+1}, ...
		training_mean2X2_pool_matrix, ...
		model_mean2X2_pool_array{i_Sparse, num_target_classes+1}, ...
		['-b 1'], 'col');
      ...
      if max4X4_pool_flag
	disp(["apply max4X4 pool model", " using ", Sparse_list{i_Sparse,2}])
	[predicted_label_max4X4_pool_array{i_Sparse, num_target_classes+1}, ...
	 accuracy_max4X4_pool_array{i_Sparse, num_target_classes+1}, ...
	 prob_values_max4X4_pool_array{i_Sparse, num_target_classes+1}] = ...
	predict(training_label_vector{num_target_classes+1}, ...
		training_max4X4_pool_matrix, ...
		model_max4X4_pool_array{i_Sparse, num_target_classes+1}, ...
		['-b 1'], 'col');
      ...
      if mean4X4_pool_flag
	disp(["apply mean4X4 pool model", " using ", Sparse_list{i_Sparse,2}])
	[predicted_label_mean4X4_pool_array{i_Sparse, num_target_classes+1}, ...
	 accuracy_mean4X4_pool_array{i_Sparse, num_target_classes+1}, ...
	 prob_values_mean4X4_pool_array{i_Sparse, num_target_classes+1}] = ...
	predict(training_label_vector{num_target_classes+1}, ...
		training_mean4X4_pool_matrix, ...
		model_mean4X4_pool_array{i_Sparse, num_target_classes+1}, ...
		['-b 1'], 'col');
	...
	for i_target_classID = 1 : num_target_classes
	  ...
	  if exclusive_flag == 0
		...
		training_mean_pool_neg = training_mean_pool_matrix(:, neg_labels_ndx{i_target_classID});
	      ...
	      if max2X2_pool_flag
		training_max2X2_pool_pos = training_max2X2_pool_matrix(:,pos_labels_ndx{i_target_classID});
		training_max2X2_pool_neg = training_max2X2_pool_matrix(:, neg_labels_ndx{i_target_classID});
	      endif
	      if mean2X2_pool_flag
		training_mean2X2_pool_pos = training_mean2X2_pool_matrix(:,pos_labels_ndx{i_target_classID});
		training_mean2X2_pool_neg = training_mean2X2_pool_matrix(:, neg_labels_ndx{i_target_classID});
	      endif
	      if max4X4_pool_flag
		training_max4X4_pool_pos = training_max4X4_pool_matrix(:,pos_labels_ndx{i_target_classID});
		training_max4X4_pool_neg = training_max4X4_pool_matrix(:, neg_labels_ndx{i_target_classID});
	      endif
	      if mean4X4_pool_flag
		training_mean4X4_pool_pos = training_mean4X4_pool_matrix(:,pos_labels_ndx{i_target_classID});
		training_mean4X4_pool_neg = training_mean4X4_pool_matrix(:, neg_labels_ndx{i_target_classID});
	      endif
		...
		training_mean_pool_neg_long = training_mean_pool_matrix(:, neg_labels_ndx_long{i_target_classID});
	      ...
	      if max2X2_pool_flag
		training_max2X2_pool_pos_long = training_max2X2_pool_matrix(:,pos_labels_ndx_long{i_target_classID});
		training_max2X2_pool_neg_long = training_max2X2_pool_matrix(:, neg_labels_ndx_long{i_target_classID});
	      endif
	      if mean2X2_pool_flag
		training_mean2X2_pool_pos_long = training_mean2X2_pool_matrix(:,pos_labels_ndx_long{i_target_classID});
		training_mean2X2_pool_neg_long = training_mean2X2_pool_matrix(:, neg_labels_ndx_long{i_target_classID});
	      endif
	      if max4X4_pool_flag
		training_max4X4_pool_pos_long = training_max4X4_pool_matrix(:,pos_labels_ndx_long{i_target_classID});
		training_max4X4_pool_neg_long = training_max4X4_pool_matrix(:, neg_labels_ndx_long{i_target_classID});
	      endif
	      if mean4X4_pool_flag
		training_mean4X4_pool_pos_long = training_mean4X4_pool_matrix(:,pos_labels_ndx_long{i_target_classID});
		training_mean4X4_pool_neg_long = training_mean4X4_pool_matrix(:, neg_labels_ndx_long{i_target_classID});
	      endif
	  ...
	  if exclusive_flag == 2
	    ...
	    if hist_pool_flag
	      disp(["cross-validation for hist model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	    ...
	    if max_pool_flag
	      disp(["cross-validation for max model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	    ...
	    if mean_pool_flag
	      disp(["cross-validation for mean model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	    ...
	    if max2X2_pool_flag
	      disp(["cross-validation for max2X2 model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	      xval_model_max2X2_pool_array{i_Sparse, i_target_classID} = ...
	      train(training_label_vector{i_target_classID}, ...
		    training_max2X2_pool_matrix, ...
		    liblinear_xval_options_str, 'col');
	    endif
	    if mean2X2_pool_flag
	      disp(["cross-validation for mean2X2 model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	      xval_model_mean2X2_pool_array{i_Sparse, i_target_classID} = ...
	      train(training_label_vector{i_target_classID}, ...
		    training_mean2X2_pool_matrix, ...
		    liblinear_xval_options_str, 'col');
	    endif
	    if max4X4_pool_flag
	      disp(["cross-validation for max4X4 model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	      xval_model_max4X4_pool_array{i_Sparse, i_target_classID} = ...
	      train(training_label_vector{i_target_classID}, ...
		    training_max4X4_pool_matrix, ...
		    liblinear_xval_options_str, 'col');
	    endif
	    if mean4X4_pool_flag
	      disp(["cross-validation for mean4X4 model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	      xval_model_mean4X4_pool_array{i_Sparse, i_target_classID} = ...
	      train(training_label_vector{i_target_classID}, ...
		    training_mean4X4_pool_matrix, ...
		    liblinear_xval_options_str, 'col');
	    endif
	  ...
	  if exclusive_flag == 2
	    ...
	    if hist_pool_flag
	      disp(["training hist model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	  ...
	  if hist_pool_flag
	    disp(["spply hist model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	  ...
	  if max_pool_flag
	    disp(["spply max model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	  ...
	  if mean_pool_flag
	    disp(["spply mean model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	  ...
	  if max2X2_pool_flag
	    disp(["spply max2X2 model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	    [predicted_label_max2X2_pool_array{i_Sparse, i_target_classID}, ...
	     accuracy_max2X2_pool_array{i_Sparse, i_target_classID}, ...
	     prob_values_max2X2_pool_array{i_Sparse, i_target_classID}] = ...
	    predict(training_label_vector{i_target_classID}, ...
		    training_max2X2_pool_matrix, ...
		    model_max2X2_pool_array{i_Sparse, i_target_classID}, ...
		    ['-b 1'], 'col');
	  ...
	  if mean2X2_pool_flag
	    disp(["spply mean2X2 model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	    [predicted_label_mean2X2_pool_array{i_Sparse, i_target_classID}, ...
	     accuracy_mean2X2_pool_array{i_Sparse, i_target_classID}, ...
	     prob_values_mean2X2_pool_array{i_Sparse, i_target_classID}] = ...
	    predict(training_label_vector{i_target_classID}, ...
		    training_mean2X2_pool_matrix, ...
		    model_mean2X2_pool_array{i_Sparse, i_target_classID}, ...
		    ['-b 1'], 'col');
	  ...
	  if max4X4_pool_flag
	    disp(["spply max4X4 model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	    [predicted_label_max4X4_pool_array{i_Sparse, i_target_classID}, ...
	     accuracy_max4X4_pool_array{i_Sparse, i_target_classID}, ...
	     prob_values_max4X4_pool_array{i_Sparse, i_target_classID}] = ...
	    predict(training_label_vector{i_target_classID}, ...
		    training_max4X4_pool_matrix, ...
		    model_max4X4_pool_array{i_Sparse, i_target_classID}, ...
		    ['-b 1'], 'col');
	  ...
	  if mean4X4_pool_flag
	    disp(["spply mean4X4 model for target class = ", target_classes{i_target_classID}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_classID))]);
	    [predicted_label_mean4X4_pool_array{i_Sparse, i_target_classID}, ...
	     accuracy_mean4X4_pool_array{i_Sparse, i_target_classID}, ...
	     prob_values_mean4X4_pool_array{i_Sparse, i_target_classID}] = ...
	    predict(training_label_vector{i_target_classID}, ...
		    training_mean4X4_pool_matrix, ...
		    model_mean4X4_pool_array{i_Sparse, i_target_classID}, ...
		    ['-b 1'], 'col');
	...
	for i_target_classID = 1 : num_target_classes
	  ...
	  if exclusive_flag == 2
		...
		train([training_label_pos_long{i_target_classID}; training_label_neg_long{i_target_classID}], ...
	      ...
	      if max2X2_pool_flag
		xval_model_max2X2_pool_array{num_Sparse_list+1, i_target_classID} = ...
		train([training_label_pos_long{i_target_classID}; training_label_neg_long{i_target_classID}], ...
		      training_max2X2_pool_matrix_array_long{num_Sparse_list+1, i_target_classID}, ...
		      liblinear_xval_options_str, 'col');
	      endif
	      if mean2X2_pool_flag
		xval_model_mean2X2_pool_array{num_Sparse_list+1, i_target_classID} = ...
		train([training_label_pos_long{i_target_classID}; training_label_neg_long{i_target_classID}], ...
		      training_mean2X2_pool_matrix_array_long{num_Sparse_list+1, i_target_classID}, ...
		      liblinear_xval_options_str, 'col');
	      endif
	      if max4X4_pool_flag
		xval_model_max4X4_pool_array{num_Sparse_list+1, i_target_classID} = ...
		train([training_label_pos_long{i_target_classID}; training_label_neg_long{i_target_classID}], ...
		      training_max4X4_pool_matrix_array_long{num_Sparse_list+1, i_target_classID}, ...
		      liblinear_xval_options_str, 'col');
	      endif
	      if mean4X4_pool_flag
		xval_model_mean4X4_pool_array{num_Sparse_list+1, i_target_classID} = ...
		train([training_label_pos_long{i_target_classID}; training_label_neg_long{i_target_classID}], ...
		      training_mean4X4_pool_matrix_array_long{num_Sparse_list+1, i_target_classID}, ...
		      liblinear_xval_options_str, 'col');
	      endif
...
if plot_flag
  ...
  if max2X2_pool_flag
    i_pool = i_pool + 1;
    pool_types{i_pool} = "max2X2";
  endif
  if mean2X2_pool_flag
    i_pool = i_pool + 1;
    pool_types{i_pool} = "mean2X2";
  endif
  if max4X4_pool_flag
    i_pool = i_pool + 1;
    pool_types{i_pool} = "max4X4";
  endif
  if mean4X4_pool_flag
    i_pool = i_pool + 1;
    pool_types{i_pool} = "mean4X4";
  endif
  ...
  for i_pool = 1:num_pool
	...
	elseif strcmp(pool_types{i_pool},"max4X4") && max4X4_pool_flag
	  num_hit = sum((predicted_label_max4X4_pool_array{i_Sparse, k_target_classID}(:) == j_target_classID) & (training_label_vector{k_target_classID}(:) == j_target_classID));
	  num_false = sum((predicted_label_max4X4_pool_array{i_Sparse, k_target_classID}(:) == j_target_classID) & (training_label_vector{k_target_classID}(:) ~= j_target_classID));
	elseif strcmp(pool_types{i_pool},"mean4X4") && mean4X4_pool_flag
	  num_hit = sum((predicted_label_mean4X4_pool_array{i_Sparse, k_target_classID}(:) == j_target_classID) & (training_label_vector{k_target_classID}(:) == j_target_classID));
	  num_false = sum((predicted_label_mean4X4_pool_array{i_Sparse, k_target_classID}(:) == j_target_classID) & (training_label_vector{k_target_classID}(:) ~= j_target_classID));
	endif %% pool_types
...
for i_Sparse = 1 : (num_Sparse_list + (num_Sparse_list > 1))
  ...
  pred_classID_str = ["GroundTruthRecon", Sparse_list{i_Sparse,2}];
  pred_classID_str_length = length(pred_classID_str);
  pred_classID_file_glob = glob([output_dir, filesep, pred_classID_str, ".pvp"]);
  pred_classID_file_glob2X2 = glob([output_dir, filesep, pred_classID_str, "2X2", ".pvp"]);
  pred_classID_file_glob = [pred_classID_file_glob; pred_classID_file_glob2X2];
  pred_classID_file_glob4X4 = glob([output_dir, filesep, pred_classID_str, "4X4", ".pvp"]);
  pred_classID_file_glob = [pred_classID_file_glob; pred_classID_file_glob4X4];
# pred_classID_file = fullfile([output_dir, filesep, pred_classID_str, ".pvp"])
# pred_classID_file2X2 = fullfile([output_dir, filesep, pred_classID_str, "2X2", ".pvp"])
# pred_classID_file4X4 = fullfile([output_dir, filesep, pred_classID_str, "4X4", ".pvp"])
  pred_data = [];
  SLP_flag = (true && exist(pred_classID_file_glob))*length(pred_classID_file_glob);
  if SLP_flag
    for i_SLP = 1 : length(pred_classID_file_glob)
      [pred_classID_dir, pred_classID_filename, pred_classID_ext, ~] = fileparts(pred_classID_file_glob{i_SLP});
      pred_classID_str_ndx = strfind(pred_classID_filename, Sparse_list{i_Sparse,2});
      pool_types{num_pool+i_SLP} = ["SLP", pred_classID_filename(pred_classID_str_ndx(1):length(pred_classID_filename))];
    endfor
  endif
  for i_pool = 1 : num_pool + SLP_flag  %% loop over SLP result, if present
    ...
    elseif i_pool > num_pool  %% use SLP ground truth prediction
      i_SLP = i_pool - num_pool;
      pred_classID_file = pred_classID_file_glob{i_SLP};
	...
	for i_target_classID = 1 : num_target_classes
	  if exclusive_flag == 0
	    ...
	    elseif strcmp(pool_types{i_pool},"max2X2")
	      target_classID_tmp = prob_values_max2X2_pool_array{i_Sparse, i_target_classID}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny);
	      target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	    elseif strcmp(pool_types{i_pool},"mean2X2")
	      target_classID_tmp = prob_values_mean2X2_pool_array{i_Sparse, i_target_classID}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny);
	      target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	    elseif strcmp(pool_types{i_pool},"max4X4")
	      target_classID_tmp = prob_values_max4X4_pool_array{i_Sparse, i_target_classID}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny);
	      target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	    elseif strcmp(pool_types{i_pool},"mean4X4")
	      target_classID_tmp = prob_values_mean4X4_pool_array{i_Sparse, i_target_classID}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny);
	      target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	    endif %% pool_types
	  elseif exclusive_flag == 1
	    if strcmp(pool_types{i_pool},"hist")
	    ...
	    elseif strcmp(pool_types{i_pool},"mean2X2")
	      ...
	      target_classID_cube(:,:,i_target_classID) = target_classID_tmp;
	    elseif strcmp(pool_types{i_pool},"max4X4")
	      label_ndx =  find(model_max4X4_pool_array{i_Sparse, num_target_classes+1}.Label == i_target_classID);
	      target_classID_tmp = prob_values_max4X4_pool_array{i_Sparse, i_target_classID}(1+(i_frame-1): i_frame, label_ndx);
	      target_classID_cube(:,:,i_target_classID) = target_classID_tmp;
	    elseif strcmp(pool_types{i_pool},"mean4X4")
	      label_ndx =  find(model_mean4X4_pool_array{i_Sparse, num_target_classes+1}.Label == i_target_classID);
	      target_classID_tmp = prob_values_mean4X4_pool_array{i_Sparse, i_target_classID}(1+(i_frame-1): i_frame, label_ndx);
	      target_classID_cube(:,:,i_target_classID) = target_classID_tmp;
	    endif %% pool_types
	  else
	    if strcmp(pool_types{i_pool},"hist")
	    ...
	    elseif strcmp(pool_types{i_pool},"mean")
	      ...
	      target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	    elseif strcmp(pool_types{i_pool},"max2X2")
	      label_ndx =  find(model_max2X2_pool_array{i_Sparse, i_target_classID}.Label == i_target_classID);
	      target_classID_tmp = prob_values_max2X2_pool_array{i_Sparse, i_target_classID}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny, label_ndx);
	      target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	    elseif strcmp(pool_types{i_pool},"mean2X2")
	      label_ndx =  find(model_mean2X2_pool_array{i_Sparse, i_target_classID}.Label == i_target_classID);
	      target_classID_tmp = prob_values_mean2X2_pool_array{i_Sparse, i_target_classID}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny, label_ndx);
	      target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	    elseif strcmp(pool_types{i_pool},"max4X4")
	      label_ndx =  find(model_max4X4_pool_array{i_Sparse, i_target_classID}.Label == i_target_classID);
	      target_classID_tmp = prob_values_max4X4_pool_array{i_Sparse, i_target_classID}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny, label_ndx);
	      target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	    elseif strcmp(pool_types{i_pool},"mean4X4")
	      label_ndx =  find(model_mean4X4_pool_array{i_Sparse, i_target_classID}.Label == i_target_classID);
	      target_classID_tmp = prob_values_mean4X4_pool_array{i_Sparse, i_target_classID}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny, label_ndx);
	      target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	    endif %% pool_types
	  endif %% exclusive_flag
	endfor %% i_target_classID
	...
	pred_classID_confidence_thresh = zeros(length(target_class_indices),1);
	...
	pred_classID_confidences = zeros(size(target_classID_cube));	
	for i_target_classID = 1 : num_target_classes

	  pred_classID_confidence_bins = 1+floor((target_classID_cube(:,:,i_target_classID) - classID_hist_bins(1)) ./ classID_bin_width);
	...
	for i_target_classID = 1 : num_target_classes
	  target_confidences{pred_classID_sorted_ndx(i_target_classID), 1} = [target_classes{pred_classID_sorted_ndx(i_target_classID)}, ...
									      ', ', 'confidence = ', num2str(pred_classID_max_confidence(pred_classID_sorted_ndx(i_target_classID))), ...
									      ', ', 'confidence_thresh = ', num2str(pred_classID_confidence_thresh(pred_classID_sorted_ndx(i_target_classID))), ...
