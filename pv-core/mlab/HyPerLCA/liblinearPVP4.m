...
target_class_indices = [7 15]+1; %%[2 6 7 14 15]+1; %%[0 1 2 3 6 7 8 12 13 14 15 19]+1; 
...
for i_target_class = 1 : num_target_classes
  training_label_vector{i_target_class} = ...
  reshape(training_label_vector_array(:,:,:,i_target_class), [ny_GT * nx_GT * num_GT_images, 1]);
  pos_labels_ndx{i_target_class} = find(training_label_vector{i_target_class});
  neg_labels_ndx_long{i_target_class} = find(~training_label_vector{i_target_class});
  neg_pos_ratio(i_target_class) = length(neg_labels_ndx_long{i_target_class})/length(pos_labels_ndx{i_target_class});
  ...
  pos_labels_ndx_long{i_target_class} = repmat(pos_labels_ndx{i_target_class}, ceil(neg_pos_ratio(i_target_class)), 1);;
  pos_labels_ndx_long{i_target_class} = pos_labels_ndx_long{i_target_class}(1:length(neg_labels_ndx_long{i_target_class}));
  ...
  [neg_labels_sorted, neg_labels_rank] = sort(rand(length(neg_labels_ndx_long{i_target_class}),1));
  neg_labels_ndx{i_target_class} = neg_labels_ndx_long{i_target_class}(neg_labels_rank(1:length(pos_labels_ndx{i_target_class})));
  %%endif %% repmat_pos_labels_flag
  training_label_pos{i_target_class} = training_label_vector{i_target_class}(pos_labels_ndx{i_target_class});
  training_label_neg{i_target_class} = training_label_vector{i_target_class}(neg_labels_ndx{i_target_class});
  training_label_pos_long{i_target_class} = training_label_vector{i_target_class}(pos_labels_ndx_long{i_target_class});
  training_label_neg_long{i_target_class} = training_label_vector{i_target_class}(neg_labels_ndx_long{i_target_class});
endfor %% class_ndx
...
for i_Sparse = 1 : (num_Sparse_list + (num_Sparse_list > 1))
  ...
  for i_target_class = 1 : num_target_classes
    disp(["training model for target class = ", target_classes{i_target_class}, " using ", Sparse_list{i_Sparse,2}, ": neg_pos_ratio = ", num2str(neg_pos_ratio(i_target_class))]);
    if i_Sparse <= num_Sparse_list	  
	...
	%%Sparse_max_pool_array{i_Sparse}{i_Sparse_frame};
      ...
      training_hist_pool_pos_long = training_hist_pool_matrix(:,pos_labels_ndx_long{i_target_class});
      training_hist_pool_neg_long = training_hist_pool_matrix(:, neg_labels_ndx_long{i_target_class});
      training_max_pool_pos_long = training_max_pool_matrix(:,pos_labels_ndx_long{i_target_class});
      training_max_pool_neg_long = training_max_pool_matrix(:, neg_labels_ndx_long{i_target_class});
      training_mean_pool_pos_long = training_mean_pool_matrix(:,pos_labels_ndx_long{i_target_class});
      training_mean_pool_neg_long = training_mean_pool_matrix(:, neg_labels_ndx_long{i_target_class});
      %%training_combo_pool_pos_long = training_combo_pool_matrix(:,pos_labels_ndx_long{i_target_class});
      %%training_combo_pool_neg_long = training_combo_pool_matrix(:, neg_labels_ndx_long{i_target_class});
      ...
      train_long_flag = false;
      if train_long_flag 
	model_hist_pool_array{i_Sparse, i_target_class} = ...
	train([training_label_pos_long{i_target_class}; training_label_neg_long{i_target_class}], ...
	      [training_hist_pool_pos_long, training_hist_pool_neg_long], ...
	      ['-s 0', '-c ', num2str(xval_model_hist_pool_array{i_Sparse, i_target_class}(1)), '-B 1'], 'col');
	model_max_pool_array{i_Sparse, i_target_class} = ...
	train([training_label_pos_long{i_target_class}; training_label_neg_long{i_target_class}], ...
	      [training_max_pool_pos_long, training_max_pool_neg_long], ...
	      ['-s 0', '-c ', num2str(xval_model_max_pool_array{i_Sparse, i_target_class}(1)), '-B 1'], 'col');
	model_mean_pool_array{i_Sparse, i_target_class} = ...
	train([training_label_pos_long{i_target_class}; training_label_neg_long{i_target_class}], ...
	      [training_mean_pool_pos_long, training_mean_pool_neg_long], ...
	      ['-s 0', '-c ', num2str(xval_model_mean_pool_array{i_Sparse, i_target_class}(1)), '-B 1'], 'col');
	%%model_combo_pool_array{i_Sparse, i_target_class} = ...
	%%train([training_label_pos_long{i_target_class}; training_label_neg_long{i_target_class}], ...
	%%	[training_combo_pool_pos_long, training_combo_pool_neg_long], ...
	%%	['-s 0', '-c ', num2str(xval_model_combo_pool_array{i_Sparse, i_target_class}(1)), '-B 1'], 'col');
      else
	model_hist_pool_array{i_Sparse, i_target_class} = ...
	train([training_label_pos{i_target_class}; training_label_neg{i_target_class}], ...
	      [training_hist_pool_pos, training_hist_pool_neg], ...
	...
	%%	['-s 0', '-c ', num2str(xval_model_combo_pool_array{i_Sparse, i_target_class}(1)), '-B 1'], 'col');
      ...
      [predicted_label_hist_pool_array{i_Sparse, i_target_class}, ...
       accuracy_hist_pool_array{i_Sparse, i_target_class}, ...
       prob_values_hist_pool_array{i_Sparse, i_target_class}] = ...
      predict(training_label_vector{i_target_class}, ...
	      training_hist_pool_matrix, ...
	      model_hist_pool_array{i_Sparse, i_target_class}, ...
	      ['-b 1'], 'col');
      [predicted_label_max_pool_array{i_Sparse, i_target_class}, ...
       accuracy_max_pool_array{i_Sparse, i_target_class}, ...
       prob_values_max_pool_array{i_Sparse, i_target_class}] = ...
      predict(training_label_vector{i_target_class}, ...
	      training_max_pool_matrix, ...
	      model_max_pool_array{i_Sparse, i_target_class}, ...
	      ['-b 1'], 'col');
      [predicted_label_mean_pool_array{i_Sparse, i_target_class}, ...
       accuracy_mean_pool_array{i_Sparse, i_target_class}, ...
       prob_values_mean_pool_array{i_Sparse, i_target_class}] = ...
      predict(training_label_vector{i_target_class}, ...
	      training_mean_pool_matrix, ...
	      model_mean_pool_array{i_Sparse, i_target_class}, ...
	      ['-b 1'], 'col');
	...
	if i_Sparse == 1
	  ...
	  training_hist_pool_matrix_array_long{num_Sparse_list+1, i_target_class} = ...
	  [training_hist_pool_pos_long, training_hist_pool_neg_long];
	  training_max_pool_matrix_array_long{num_Sparse_list+1, i_target_class} = ...
	  [training_max_pool_pos_long, training_max_pool_neg_long];
	  training_mean_pool_matrix_array_long{num_Sparse_list+1, i_target_class} = ...
	  [training_mean_pool_pos_long, training_mean_pool_neg_long];
	  %%training_combo_pool_matrix_array_long{num_Sparse_list+1, i_target_class} = ...
	  %%[training_combo_pool_pos_long, training_combo_pool_neg_long];
	  
	  training_hist_pool_matrix_array_predict{num_Sparse_list+1, i_target_class} = ...
	  training_hist_pool_matrix;
	  training_max_pool_matrix_array_predict{num_Sparse_list+1, i_target_class} = ...
	  training_max_pool_matrix;
	  training_mean_pool_matrix_array_predict{num_Sparse_list+1, i_target_class} = ...
	  training_mean_pool_matrix;
	  %%training_combo_pool_matrix_array_predict{num_Sparse_list+1, i_target_class} = ...
	  %%training_combo_pool_matrix;
	else
	  training_hist_pool_matrix_array{num_Sparse_list+1, i_target_class} = ...
	  [training_hist_pool_matrix_array{num_Sparse_list+1, i_target_class}; ...
	...
	train([training_label_pos_long{i_target_class}; training_label_neg_long{i_target_class}], ...
	      training_hist_pool_matrix_array_long{num_Sparse_list+1, i_target_class}, ...
	      ['-s 0', '-c ', num2str(xval_model_hist_pool_array{num_Sparse_list+1, i_target_class}(1)), '-B 1'], 'col');
	model_max_pool_array{num_Sparse_list+1, i_target_class} = ...
	train([training_label_pos_long{i_target_class}; training_label_neg_long{i_target_class}], ...
	      training_max_pool_matrix_array_long{num_Sparse_list+1, i_target_class}, ...
	      ['-s 0', '-c ', num2str(xval_model_max_pool_array{num_Sparse_list+1, i_target_class}(1)), '-B 1'], 'col');
	model_mean_pool_array{num_Sparse_list+1, i_target_class} = ...
	train([training_label_pos_long{i_target_class}; training_label_neg_long{i_target_class}], ...
	      training_mean_pool_matrix_array_long{num_Sparse_list+1, i_target_class}, ...
	      ['-s 0', '-c ', num2str(xval_model_mean_pool_array{num_Sparse_list+1, i_target_class}(1)), '-B 1'], 'col');
      else
	model_hist_pool_array{num_Sparse_list+1, i_target_class} = ...
	train([training_label_pos{i_target_class}; training_label_neg{i_target_class}], ...
	      training_hist_pool_matrix_array{num_Sparse_list+1, i_target_class}, ...
...
for i_Sparse = 1 : (num_Sparse_list + (num_Sparse_list > 1))
  ...
  for i_pool = 1 : num_pool + SLP_flag  %% loop over SLP result, if present
    ...
    if i_pool > num_pool
      ...
      min_prob = min(prob_values_hist_pool_array{i_Sparse, i_target_class}(:));
      max_prob = max(prob_values_hist_pool_array{i_Sparse, i_target_class}(:));
	...
	for i_target_classID = 1 : num_target_classes
	  if strcmp(pool_types{i_pool},"hist")
	    target_classID_tmp = prob_values_hist_pool_array{i_Sparse, i_target_class}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny);
	    target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	  elseif strcmp(pool_types{i_pool},"max")
	    target_classID_tmp = prob_values_max_pool_array{i_Sparse, i_target_class}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny);
	    target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	  elseif strcmp(pool_types{i_pool},"mean")
	    target_classID_tmp = prob_values_mean_pool_array{i_Sparse, i_target_class}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny);
	    target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	  elseif strcmp(pool_types{i_pool},"combo")
	    target_classID_tmp = prob_values_combo_pool_array{i_Sparse, i_target_class}(1+(i_frame-1) * GT_hdr.nx * GT_hdr.ny : i_frame * GT_hdr.nx * GT_hdr.ny);
	    target_classID_cube(:,:,i_target_classID) = reshape(target_classID_tmp, GT_hdr.ny, GT_hdr.nx );
	  endif %% pool_types
