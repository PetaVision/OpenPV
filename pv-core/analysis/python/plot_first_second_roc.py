...
#######
      ...
      blen = np.shape(A1pos2)[0]
      dif = blen - alen
      if alen < blen:
         print "digitalism"
         for i in range(dif):
            A1pos2 = np.delete(A1pos2, -1, axis=0)

      dif2 = alen - blen
      if alen > blen:
         print "digitalism"
         for i in range(dif2):
            A1pos = np.delete(A1pos, -1, axis=0)



      print "pos shape = ", np.shape(A1pos)
      print "A1pos = ", A1pos
      print "pos shape2 = ", np.shape(A1pos2)
      print "A1pos2 = ", A1pos2
      ...
      for k in range(zerange):    ####### range(step)
         ...
         A1t2 = np.zeros((1, np.shape(A1pos2)[0]))
...
#####
         for g in range(np.shape(A1pos)[0]):
            ...
            for h in range(len(A1A)):
               if A1A[h] == ((lenofo * i) + j):
                  A1t[0, g] += 1
         for g in range(np.shape(A1pos2)[0]):
            w = A1pos2[g]
            i = w[0]
            j = w[1]
            for h in range(len(A1A)):
      ...
      np.savetxt("roc-info-first-second.txt", A1q, fmt='%d', delimiter = ';')        
      sys.exit()
