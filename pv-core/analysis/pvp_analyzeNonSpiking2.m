function [] = pvp_analyzeNonSpiking2(target_path, ...
				     distractor_path, ...
				     num_trials, first_trial, last_trial, skip_trial)

  global WORKSPACE_PATH
  WORKSPACE_PATH = ...
      '~/workspace-indigo/';

  %% wrapper function for analying non-spiking PetaVision output
  if nargin < 1 || ~exist(target_path) || isempty(target_path)
    target_path = ...
	[machine_path "ODD/input/imageNet/DoG_Mask/test/cat_target"];
  endif
...
num_trials = 0; %%600; %% cannot exceed ~1024 for 256x256 image because
...
if num_trials > num_single_trials || RAW_HIST_FLAG
  distractor_path = ...
    [machine_path "ODD/input/imageNet/DoG_Mask/test/dog_distractor"];
