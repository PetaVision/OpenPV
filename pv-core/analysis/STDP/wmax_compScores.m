function [S, F] = wmax_compScores(fname_last, xScale, yScale)
...
for f=1:numel(fname_last)
    % score for each neuron (max overlap value)
    S_array = []; % N x patch_size array where N = NX * NY 
    % feature learned (max feature overlap)
    F_array = []; % N x patch_size array where N = NX * NY 
        ...
        for i=1:NXscaled
            if ~feof(fid{f})
                ...
                if(~isempty(w) & nItems ~= 0)
                    ...
                    % comp learning score
                    [Overlap, FeatureI ]= max(w' * Features ) ;
                    %fprintf('%d %d %f  %d\n', j, i, Overlap, FeatureI);
                    S_array(k) = Overlap;
                    F_array(k) = FeatureI;
                    %pause
                end
