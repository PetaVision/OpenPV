...
int BBFindConfRemapProbe::ioParamsFillGroup(enum ParamsIOFlag ioFlag) {
...
void BBFindConfRemapProbe::ioParam_imageLayer(enum ParamsIOFlag ioFlag) {
   parent->ioParamStringRequired(ioFlag, name, "imageLayer", &imageLayerName);
}

void BBFindConfRemapProbe::ioParam_reconLayer(enum ParamsIOFlag ioFlag) {
   parent->ioParamStringRequired(ioFlag, name, "reconLayer", &reconLayerName);
}

void BBFindConfRemapProbe::ioParam_classNamesFile(enum ParamsIOFlag ioFlag) {
   parent->ioParamString(ioFlag, name, "classNamesFile", &classNamesFile, "");
}

void BBFindConfRemapProbe::ioParam_minBoundingBoxWidth(enum ParamsIOFlag ioFlag) {
...
void BBFindConfRemapProbe::ioParam_minBoundingBoxHeight(enum ParamsIOFlag ioFlag) {
...
void BBFindConfRemapProbe::ioParam_drawMontage(enum ParamsIOFlag ioFlag) {
...
void BBFindConfRemapProbe::ioParam_heatMapMontageDir(enum ParamsIOFlag ioFlag) {
...
void BBFindConfRemapProbe::ioParam_heatMapThreshold(enum ParamsIOFlag ioFlag) {
   assert(!parent->parameters()->presentAndNotBeenRead(this->getName(), "drawMontage"));
   if (drawMontage) {
...
void BBFindConfRemapProbe::ioParam_heatMapMaximum(enum ParamsIOFlag ioFlag) {
   assert(!parent->parameters()->presentAndNotBeenRead(this->getName(), "drawMontage"));
   if (drawMontage) {
...
void BBFindConfRemapProbe::ioParam_imageBlendCoeff(enum ParamsIOFlag ioFlag) {
   assert(!parent->parameters()->presentAndNotBeenRead(this->getName(), "drawMontage"));
   if (drawMontage) {
...
void BBFindConfRemapProbe::ioParam_boundingBoxLineWidth(enum ParamsIOFlag ioFlag) {
   assert(!parent->parameters()->presentAndNotBeenRead(this->getName(), "drawMontage"));
   if (drawMontage) {
...
void BBFindConfRemapProbe::ioParam_displayCommand(enum ParamsIOFlag ioFlag) {
   assert(!parent->parameters()->presentAndNotBeenRead(this->getName(), "drawMontage"));
   if (drawMontage) {
...
int BBFindConfRemapProbe::outputState(double timevalue) {
   ...
   if (getTextOutputFlag() && outputstream && outputstream->fp) {
      ...
      for (int b=0; b<nbatch; b++) {
         if (nbatch>1) {
            fprintf(outputstream->fp, "  Batch element %d\n", b);
         }
         for(std::vector<LocalizationData>::iterator it = detectionS[b].begin(); it < detectionS[b].end(); it++){
            LocalizationData const& L = *it;
            fprintf(outputstream->fp, "Time %f, ", timevalue);
            if (nbatch>1) { fprintf(outputstream->fp, "batch element %d, ", b); }
            fprintf(outputstream->fp, "\"%s\", score %f, bounding box x=[%d,%d), y=[%d,%d)\n",
