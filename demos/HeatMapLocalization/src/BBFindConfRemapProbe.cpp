...
BBFindConfRemapProbe::BBFindConfRemapProbe() { initialize_base(); }

int BBFindConfRemapProbe::initialize_base() { return PV_SUCCESS; }
...
void BBFindConfRemapProbe::ioParam_heatMapThreshold(enum PV::ParamsIOFlag ioFlag) {
   ...
   if (drawMontage) {
      parent->parameters()->ioParamArray(
            ioFlag, name, "heatMapThreshold", &heatMapThreshold, &numHeatMapThresholds);
   }
}
...
void BBFindConfRemapProbe::setOutputFilenameBase(char const *fn) {
   ...
   if (fnString.empty()) {
      if (parent->columnId() == 0) {
         fprintf(
               stderr,
               "LocalizationProbe::setOutputFilenameBase error: string \"%s\" is empty after "
               "removing directory and extension.\n",
               fn);
      }
      status             = PV_FAILURE;
      ...
      if (outputFilenameBase == NULL) {
         fprintf(
               stderr,
               "LocalizationProbe::setOutputFilenameBase failed with filename \"%s\": %s\n",
               fn,
               strerror(errno));
         exit(EXIT_FAILURE);
      }
...
int BBFindConfRemapProbe::communicateInitInfo(CommunicateInitInfoMessage const *message) {
   ...
   if (targetBBFindConfRemapLayer == NULL) {
      if (parent->columnId() == 0) {
         fprintf(
               stderr,
               "%s error: targetLayer \"%s\" must be a BBFindLayer.\n",
               getDescription_c(),
               this->targetName);
      }
...
void BBFindConfRemapProbe::setLayerFromParam(
   ...
   if (l == NULL) {
      if (parent->columnId() == 0) {
         fprintf(
               stderr,
               "%s error: %s \"%s\" does not refer to a layer in the column.\n",
               getDescription_c(),
               layerType,
               layerName);
      }
   ...
   if (drawMontage && nf != 3) {
      if (parent->columnId() == 0) {
         fprintf(
               stderr,
               "%s error: If the drawMontage flag is set, the %s must have exactly three features "
               "(\"%s\" has %d).\n",
               getDescription_c(),
               layerType,
               layerName,
...
int BBFindConfRemapProbe::allocateDataStructures() {
   ...
   if (parent->columnId() == 0 && drawMontage) {
      montageImage = (unsigned char *)pvCallocError(
            imageMontageSize,
            sizeof(*montageImage),
            "%s allocation for heat map montage image\n",
            getDescription_c());
      montageImageComm = (unsigned char *)pvCallocError(
            imageLocalSize,
            sizeof(*montageImageComm),
            "%s allocation for MPI communication of heat map montage image\n",
            getDescription_c());
   }
   montageImageLocal = (unsigned char *)pvCallocError(
         imageLocalSize,
         sizeof(*montageImageLocal),
         "%s allocation for heat map image in rank %d\n",
         getDescription_c(),
         parent->columnId());
   grayScaleImage = (float *)pvCallocError(
         imageLoc->nx * imageLoc->ny,
         sizeof(float),
...
int BBFindConfRemapProbe::calcValues(double timevalue) {
   std::vector<BBFind> const &boundingboxFinder =
         targetBBFindConfRemapLayer->getBoundingBoxFinder();
   assert(boundingboxFinder.size() == getNumValues());
   assert(detectionS.size() == getNumValues());
   for (int b = 0; b < getNumValues(); b++) {
      detectionS[b].clear();
      BBFind boundingboxFinder =
            targetBBFindConfRemapLayer
                  ->getBoundingBoxFinder()[b]; // TODO: should be able to declare as const
   ...
   if (getTextOutputFlag() && getOutputStream()) {
      ...
      for (int b = 0; b < nbatch; b++) {
         ...
         for (auto const &L : detectionS[b]) {
            ...
            if (nbatch > 1) {
               getOutputStream()->printf("batch element %d, ", b);
            }
            getOutputStream()->printf(
                  "\"%s\", score %f, bounding box x=[%d,%d), y=[%d,%d)\n",
                  classNames[L.feature],
                  L.score,
                  L.left,
                  L.right,
                  L.top,
...
void BBFindConfRemapProbe::drawTextOnMontage(
...
void BBFindConfRemapProbe::drawTextIntoFile(
   ...
   convertCmd << "convert -depth 8 -background \"" << backgroundColor << "\" -fill \"" << textColor
              << "\" -size " << width << "x" << height << " -pointsize 24 -gravity center label:\""
   ...
   if (status != 0) {
      ...
      fprintf(
            stderr,
            "%s error creating label file \"%s\": ImageMagick convert returned %d.\n",
            getDescription_c(),
            labelFilename,
            WEXITSTATUS(status));
      exit(EXIT_FAILURE);
...
void BBFindConfRemapProbe::insertFileIntoMontage(
      char const *labelFilename,
      int xOffset,
      int yOffset,
      int xExpectedSize,
   ...
   if (dataset == NULL) {
      ...
      fprintf(
            stderr,
            "%s error opening label file \"%s\" for reading.\n",
            getDescription_c(),
            labelFilename);
      exit(EXIT_FAILURE);
   }
   ...
   if (xLabelSize != xExpectedSize || yLabelSize != yExpectedSize) {
      ...
      fprintf(
            stderr,
            "%s error: label files \"%s\" has dimensions %dx%d (expected %dx%d)\n",
            getDescription_c(),
            labelFilename,
            xLabelSize,
            yLabelSize,
...
void BBFindConfRemapProbe::insertImageIntoMontage(
      int xStart,
      int yStart,
