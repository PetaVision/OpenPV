...
#define CONVTABLEHEADERSIZE                                                                        \
   (sizeof(char) * (size_t)8 + sizeof(int) + sizeof(int) + sizeof(float) + sizeof(float))
...
ConvertFromTable::ConvertFromTable() { initialize_base(); }
...
void ConvertFromTable::ioParam_dataFile(enum PV::ParamsIOFlag ioFlag) {
   parameters()->ioParamStringRequired(ioFlag, name, "dataFile", &dataFile);
}
...
int ConvertFromTable::loadConversionTable() {
   ...
   if (parent->globalRank() == 0) {
      ...
      if (status != 0) {
         Fatal().printf(
               "%s: unable to seek to end off dataFile \"%s\": %s\n",
               getDescription_c(),
               dataFile,
               strerror(errno));
      }
      ...
      if (fileLength < 0) {
         Fatal().printf(
               "%s: unable to get length of dataFile \"%s\": %s\n",
               getDescription_c(),
               dataFile,
               strerror(errno));
      }
      if (fileLength == 0) {
         Fatal().printf("%s: dataFile \"%s\" is empty.\n", getDescription_c(), dataFile);
      }
      if (fileLength < CONVTABLEHEADERSIZE) {
         Fatal().printf(
               "%s \"%s\": dataFile \"%s\" is too small (%ld bytes; minimum is %zu bytes)\n",
               getDescription_c(),
               dataFile,
               fileLength,
               CONVTABLEHEADERSIZE);
      ...
      if (numRead != CONVTABLEHEADERSIZE) {
         Fatal().printf(
               "%s \"%s\": reading header of dataFile \"%s\": expected %zu bytes; only read %zu\n",
               getDescription_c(),
               dataFile,
               CONVTABLEHEADERSIZE,
               numRead);
      ...
      if (memcmp(hdr, "convTabl", 8)) {
         Fatal().printf(
               "%s \"%s\": dataFile \"%s\" does not have the expected header.\n",
               getDescription_c(),
               dataFile);
      }
      int correctFileSize =
            (int)(sizeof(float) * convTable.numPoints * convTable.numFeatures
                  + CONVTABLEHEADERSIZE);
      if (correctFileSize != fileLength) {
         Fatal().printf(
               "%s: dataFile \"%s\" has unexpected length (%d-by-%d %zu-byte data values with "
               "%zu-byte header, but file size is %ld\n",
               getDescription_c(),
               dataFile,
               convTable.numPoints,
               convTable.numFeatures,
               sizeof(float),
               CONVTABLEHEADERSIZE,
               fileLength);
      }
      if (convTable.numFeatures != getLayerLoc()->nf) {
         Fatal().printf(
               "%s: layer has %d features but dataFile \"%s\" has numFeatures=%d\n",
               getDescription_c(),
               getLayerLoc()->nf,
               dataFile,
               convTable.numFeatures);
      ...
      if (convTable.numFeatures < 2) {
         Fatal().printf(
               "%s: dataFile \"%s\" has numPoints=%d but must be >= 2.\n",
               getDescription_c(),
               dataFile,
               convTable.numPoints);
      }
      ...
      if (convData == NULL) {
         Fatal().printf(
               "%s: unable to allocate memory for loading dataFile \"%s\": %s.\n",
               getDescription_c(),
               dataFile,
               strerror(errno));
      }
      ...
      if (numRead != numValues) {
         Fatal().printf(
               "%s: unable to read dataFile \"%s\": expecting %zu data values but only read %zu.\n",
               getDescription_c(),
               dataFile,
               numValues,
               numRead);
...
int ConvertFromTable::updateState(double timed, double dt) {
   ...
   for (int k = 0; k < getNumNeurons(); k++) {
      ...
      if (indexa < 0) {
         ...
         A[kExt] = dataAtFeature[numIntervals]
                   + (datapoint - numIntervals)
                           * (dataAtFeature[numIntervals] - dataAtFeature[numIntervals - 1]);
      }
      else {
         A[kExt] = dataAtFeature[indexa]
                   + (datapoint - indexa) * (dataAtFeature[indexa + 1] - dataAtFeature[indexa]);
      }
   }
   return PV_SUCCESS;
}
