...
BBFindConfRemapLayer::BBFindConfRemapLayer() { initialize_base(); }
...
int BBFindConfRemapLayer::communicateInitInfo(CommunicateInitInfoMessage const *message) {
      ...
      if (imageLayer == nullptr) {
         if (parent->columnId() == 0) {
            Fatal() << getDescription_c() << ": imageLayer \"" << imageLayerName
                    << "\" does not refer to a layer in the column." << std::endl;
         }
   ...
   if (numDisplayedCategories == 0) {
      ...
      displayedCategories    = (int *)pvMallocError(
            sizeof(*displayedCategories) * (size_t)numDisplayedCategories,
            "%s: unable to allocate %d values for displayedCategories array",
            getDescription_c(),
            numDisplayedCategories);
      for (int k = 0; k < numDisplayedCategories; k++) {
         displayedCategories[k] = k + 1;
      }
   }
   else {
      for (int k = 0; k < numDisplayedCategories; k++) {
         ...
         if (cat <= 0 || cat > getLayerLoc()->nf) {
            if (parent->columnId() == 0) {
               Fatal() << getDescription_c() << ": displayedCategories element " << k + 1 << " is "
                       << cat << ", outside the range [1," << getLayerLoc()->nf << "]."
                       << std::endl;
            }
            MPI_Barrier(parent->getCommunicator()->communicator());
            pvExitFailure("");
   ...
   for (int b = 0; b < parent->getNBatch(); b++) {
      ...
      if (parent->columnId() == rootProcess) {
         ...
         for (int rank = 0; rank < icComm->commSize(); rank++) {
            int const kx0 =
                  columnFromRank(rank, icComm->numCommRows(), icComm->numCommColumns()) * nx;
            ...
            for (int y = 0; y < ny; y++) {
               ...
               memcpy(
                     &confidenceGlobal[globalStart],
                     &localPart[localStart],
                     sizeof(float) * nx * nf);
            }

            boundingboxFinder[b].giveMap(
                  BBFind::bufferToMap3(
                        confidenceGlobal,
                        nxGlobal,
                        nyGlobal,
...
#pragma omp parallel for
      ...
      for (int y = 0; y < ny; y++) {
         ...
         int kExt = kIndex(
               halo->lt, y + halo->up, 0, nx + halo->lt + halo->rt, ny + halo->dn + halo->up, nf);
...
BBFindConfRemapLayer::~BBFindConfRemapLayer() { free(displayedCategories); }
