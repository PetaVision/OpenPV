...
MaskFromMemoryBuffer::~MaskFromMemoryBuffer() { free(imageLayerName); }
...
int MaskFromMemoryBuffer::communicateInitInfo(CommunicateInitInfoMessage const *message) {
   ...
   HyPerLayer *hyperLayer =
         dynamic_cast<HyPerLayer *>(message->lookup(std::string(imageLayerName)));
   if (hyperLayer == NULL) {
      if (parent->columnId() == 0) {
         ErrorLog().printf(
               "%s: imageLayerName \"%s\" is not a layer in the HyPerCol.\n",
               getDescription_c(),
               imageLayerName);
      }
      status = PV_FAILURE;
      ...
      if (imageLayer == NULL) {
         if (parent->columnId() == 0) {
            ErrorLog().printf(
                  "%s: imageLayerName \"%s\" is not an BaseInput-derived layer.\n",
                  getDescription_c(),
                  imageLayerName);
         }
         status = PV_FAILURE;
   ...
   if (!imageLayer->getInitInfoCommunicatedFlag()) {
      if (parent->columnId() == 0) {
         InfoLog().printf(
               "%s must wait until imageLayer \"%s\" has finished its communicateInitInfo stage.\n",
               getDescription_c(),
               imageLayerName);
      }
      return PV_POSTPONE;
   ...
   if (imageLayerLoc->nx != loc->nx || imageLayerLoc->ny != loc->ny) {
      if (parent->columnId() == 0) {
         ErrorLog().printf(
               "%s: dimensions (%d-by-%d) do not agree with dimensions of image layer \"%s\" "
               "(%d-by-%d)n",
               getDescription_c(),
               loc->nx,
               loc->ny,
...
int MaskFromMemoryBuffer::updateState(double time, double dt) {
   if (imageLayer->getDataLeft() == dataLeft && imageLayer->getDataTop() == dataTop
       && imageLayer->getDataWidth() == dataRight - dataLeft
       && imageLayer->getDataHeight()
       && dataBottom - dataTop) {
