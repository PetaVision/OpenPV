...
int LocalizationBBFindProbe::ioParamsFillGroup(enum ParamsIOFlag ioFlag) {
...
int LocalizationBBFindProbe::communicateInitInfo() {
   int status = LocalizationProbe::communicateInitInfo();
   ...
   if (parent->columnId()==rootProcess) {
      ...
      for (int rank=0; rank<icComm->commSize(); rank++) {
         ...
         if (rank==0) {
            ...
            MPI_Recv(confidenceLocal, targetLayer->getNumExtended(), MPI_FLOAT, rank, 137, icComm->communicator(), MPI_STATUS_IGNORE);
         }
         ...
         for (unsigned d=0; d<rsize; d++) {
            ...
            for (list<BBFind::Rectangle>::const_iterator listiter=rlist->begin(); listiter!=rlist->end(); listiter++) {
               ...
               if (bbox.right-bbox.left>=minBoundingBoxWidth && bbox.bottom-bbox.top>=minBoundingBoxHeight) {
