...
mkdir(mask_path);
classID_dir = "classID"
classID_path = fullfile(PASCAL_VOC_path, VOC_dataset, classID_dir);
mkdir(classID_path);
...
resized_list = [VOC_dataset_path, filesep, VOC_dataset, "_padded_", orientation_type, "_list.txt"];
resized_fid = fopen(resized_list, "w", "native");
mask_list = [VOC_dataset_path, filesep, VOC_dataset, "_mask_", orientation_type, "_list.txt"];
mask_fid = fopen(mask_list, "w", "native");


%% initialize CSV file (DARPA Neovision2 format)
CSV_file = [VOC_dataset_path, filesep, VOC_dataset, "_padded_", orientation_type, ".csv"];
...
for i_raw = 1 : num_raw
  ...
  for i_bbox = 1 : num_bbox
    ...
    fputs(CSV_fid, CSV_filler_str);
    fputs(CSV_fid, raw_name);
  ...
  classID_data{num_resized} = classID_struct;
...
classID_file = fullfile(classID_path, [raw_name, "pvp"]);
writepvpsparseactivityfile(classID_file, classID_data, num_classes, resized_width, resized_height);
