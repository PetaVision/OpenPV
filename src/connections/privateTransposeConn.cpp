...
privateTransposeConn::~privateTransposeConn() {} // privateTransposeConn::~privateTransposeConn()

// privateTransposeConn initialize will be called during parentConn's communicate
int privateTransposeConn::initialize(
      const char *name,
      HyPerCol *hc,
      HyPerConn *parentConn,
      bool needWeights) {
...
int privateTransposeConn::communicateInitInfo(CommunicateInitInfoMessage const *message) {
...
int privateTransposeConn::transposeNonsharedWeights(int arborId) {
   ...
   for (int neighbor = 0; neighbor < NUM_NEIGHBORHOOD; neighbor++) {
      hasRestrictedNeighbor[neighbor] =
            neighbor != Communicator::LOCAL
            && icComm->neighborIndex(parent->columnId(), neighbor) >= 0
            && icComm->reverseDirection(parent->columnId(), neighbor) + neighbor
                     == NUM_NEIGHBORHOOD;
   }
   for (int neighbor = 0; neighbor < NUM_NEIGHBORHOOD; neighbor++) {
   ...
   for (int neighbor = 0; neighbor < NUM_NEIGHBORHOOD; neighbor++) {
      if (!hasRestrictedNeighbor[neighbor]) {
         continue;
      }
      ...
      for (int y = starty[neighbor]; y < stopy[neighbor]; y++) {
         for (int x = startx[neighbor]; x < stopx[neighbor]; x++) {
            int idxExt = kIndex(
                  ...
                  y,
                  0,
                  preLocOrig->nx + preLocOrig->halo.lt + preLocOrig->halo.rt,
                  preLocOrig->ny + preLocOrig->halo.dn + preLocOrig->halo.up,
                  preLocOrig->nf);
            ...
            if (xGlobalRes >= preLocOrig->kx0 && xGlobalRes < preLocOrig->kx0 + preLocOrig->nx
               ...
               errorMessage.printf(
                     ...
                     preLocOrig->nx);
               errorMessage.printf(
                     "Rank %d, %s, x=%d, y=%d, neighbor=%d: yGlobalRes = %d, preLocOrig->ky0 = %d, "
                     "preLocOrig->ny = %d\n",
                     parent->columnId(),
                     getDescription_c(),
                     x,
   ...
   for (int kPreRestrictedOrig = 0; kPreRestrictedOrig < nPreRestrictedOrig; kPreRestrictedOrig++) {
      int kPreExtendedOrig = kIndexExtended(
            kPreRestrictedOrig,
            preLocOrig->nx,
            preLocOrig->ny,
            preLocOrig->nf,
      ...
      for (int y = 0; y < ny; y++) {
         for (int k = 0; k < nk; k++) {
            ...
            int kPreExtendedTranspose   = kIndexExtended(
                  kPreRestrictedTranspose,
                  preLocTranspose->nx,
                  preLocTranspose->ny,
                  preLocTranspose->nf,
                  preLocTranspose->halo.lt,
            ...
            for (int y = 0; y < patch.ny; y++) {
               for (int k = 0; k < patch.nx * postConn->fPatchSize(); k++) {
                  ...
                  int transposePreGlobalXRes =
                        kxPos(transposePreGlobalRes,
                              preLocTranspose->nxGlobal,
                              preLocTranspose->nyGlobal,
                              preLocTranspose->nf);
                  ...
                  assert(
                        transposePreLocalXExt >= 0
                        && transposePreLocalXExt < preLocTranspose->nx + preLocTranspose->halo.lt
                                                         + preLocTranspose->halo.rt);
                  int transposePreGlobalYRes =
                        kyPos(transposePreGlobalRes,
                              preLocTranspose->nxGlobal,
                              preLocTranspose->nyGlobal,
                              preLocTranspose->nf);
                  ...
                  assert(
                        transposePreLocalYExt >= 0
                        && transposePreLocalYExt < preLocTranspose->ny + preLocTranspose->halo.dn
                                                         + preLocTranspose->halo.up);
                  int transposePreFeature = featureIndex(
                        transposePreGlobalRes,
                        preLocTranspose->nxGlobal,
                        preLocTranspose->nyGlobal,
                        preLocTranspose->nf);
                  int transposePreLocalExt = kIndex(
                        transposePreLocalXExt,
                        transposePreLocalYExt,
                        transposePreFeature,
                        preLocTranspose->nx + preLocTranspose->halo.lt + preLocTranspose->halo.rt,
                        preLocTranspose->ny + preLocTranspose->halo.dn + preLocTranspose->halo.up,
                        preLocTranspose->nf);

                  int transposePostGlobalRes = origPreIndex;
                  int transposePostGlobalXRes =
                        kxPos(transposePostGlobalRes,
                              preLocOrig->nxGlobal,
                              preLocOrig->nyGlobal,
                              preLocOrig->nf);
                  ...
                  int transposePostGlobalYRes =
                        kyPos(transposePostGlobalRes,
                              preLocOrig->nxGlobal,
                              preLocOrig->nyGlobal,
                              preLocOrig->nf);
                  ...
                  int transposePostLocalRes = kIndex(
                        origPreLocalXRes,
                        origPreLocalYRes,
                        f,
                        postLocTranspose->nx,
...
int privateTransposeConn::mpiexchangesize(
      int neighbor,
      int *size,
      int *startx,
      int *stopx,
   ...
   switch (neighbor) {
      case Communicator::LOCAL: assert(0); break;
...
int privateTransposeConn::transposeSharedWeights(int arborId) {
   ...
   int xscalediff = pre->getXScale() - post->getXScale();
   int yscalediff = pre->getYScale() - post->getYScale();
   // scalediff>0 means privateTransposeConn's post--that is, the postConn's pre--has a higher
   // neuron density
         ...
         for (int kyFB = 0; kyFB < nyFB; kyFB++) {
            for (int kxFB = 0; kxFB < nxFB; kxFB++) {
               for (int kfFB = 0; kfFB < nfFB; kfFB++) {
                  ...
                  // Calculate x and y position of the FB kernel, with an offset for the case of
                  // even patches
                  ...
                  // can the calls to kxPos, kyPos, featureIndex be replaced by one call to
                  // patchIndexToKernelIndex?
...
int privateTransposeConn::reduceKernels(int arborID) {
   // Values are taken from postConn.  If postConn keeps kernels synchronized, then
   // privateTransposeConn stays synchronized automatically.
