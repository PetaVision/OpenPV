...
FeedbackConn::FeedbackConn() { initialize_base(); }
...
int FeedbackConn::initialize_base() { return PV_SUCCESS; }
...
int FeedbackConn::setPreAndPostLayerNames() {
   ...
   if (params->stringPresent(name, "preLayerName")
       || params->stringPresent(name, "postLayerName")) {
      if (parent->columnId() == 0) {
         pvErrorNoExit().printf(
               "%s: FeedbackConn does not use preLayerName or postLayerName.\n",
               getDescription_c());
      }
      status = PV_FAILURE;
   }
...
int FeedbackConn::handleMissingPreAndPostLayerNames() {
   ...
   if (preLayerName == NULL || postLayerName == NULL) {
      pvError().printf(
            "%s: Rank %d process unable to allocate memory for pre and post layer names: %s",
            getDescription_c(),
            parent->columnId(),
            strerror(errno));
   }
