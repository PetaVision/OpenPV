...
int CliqueConn::calc_dW(int arborId){
   // zero dWeightPatches
    ...
    int numActiveExt = 0;
    unsigned int * activeExt = (unsigned int *) calloc(this->getPre()->getNumExtended(), sizeof(int));
    ...
    for (int kPreActive = 0; kPreActive < numActiveExt; kPreActive++) {
            ...
            for(int kClique = 0; kClique < numActiveCliques; kClique++) {
                    ...
                    for (int y = 0; y < nyPatch; y++) {
                            pvpatch_update_clique(nkPatch, (float *) (dWPatch->data + y*syPatch), cliqueProd, (float *) (aPost + y*syPostExt));
                    }
