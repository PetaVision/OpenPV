...
class Buffer {
  public:
   ...
   T const at(long k) const;

   void set(int x, int y, int feature, T value);
   void set(long k, T value);
   ...
   void insertFeatures(Buffer<T> const &insertion, int fStart);
   ...
   long getTotalElements() const { return (long)mHeight * (long)mWidth * (long)mFeatures; }
  ...
  protected:
   ...
   long index(int x, int y, int f) const;
