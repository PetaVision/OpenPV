...
// Previously, GapConn required that normalizeMethod be set to normalizeSum and that
// normalizeFromPostPerspective be set to true.
// Neither of these requirements are still present, but for now we want the defaults be what the
...
NormalizeGap::NormalizeGap() { initialize_base(); }
...
int NormalizeGap::initialize_base() { return PV_SUCCESS; }
...
void NormalizeGap::ioParam_normalizeFromPostPerspective(enum ParamsIOFlag ioFlag) {
   ...
   if (ioFlag == PARAMS_IO_READ) {
      ...
      if (parent->parameters()->present(name, "normalizeFromPostPerspective")
          && parent->columnId() == 0) {
         pvWarn().printf(
               "%s \"%s\": normalizeFromPostPerspective default is true for GapConns with "
               "normalizeMethod of normalizeSum, but the default for this parameter may change to "
               "false in a future release, to be consistent with other normalizerMethods.\n",
