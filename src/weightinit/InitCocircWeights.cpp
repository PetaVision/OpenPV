   ...
   for (int kfPost = 0; kfPost < nfPatch_tmp; kfPost++) {
      ...
      for (int jPost = 0; jPost < nyPatch_tmp; jPost++) {
         ...
         for (int iPost = 0; iPost < nxPatch_tmp; iPost++) {
            ...
            if (calcDistChordCocircKurvePreNKurvePost(
                      xDelta, yDelta, kfPost, weightParamPtr, thPost)) {
               continue;
            }
...
bool InitCocircWeights::calcDistChordCocircKurvePreNKurvePost(
   ...
   if (d2 == 0) {
      ...
      weightParamPtr->updateCocircNChord(thPost, dyP_shift, dxP, cocircKurve_shift, d2_shift);
      ...
      if (numFlanks > 1) {
         ...
         weightParamPtr->updateCocircNChord(thPost, dyP_shift2, dxP, cocircKurve_shift2, d2_shift);
