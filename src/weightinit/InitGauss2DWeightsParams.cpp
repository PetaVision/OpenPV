...
InitGauss2DWeightsParams::InitGauss2DWeightsParams() { initialize_base(); }
...
void InitGauss2DWeightsParams::ioParam_strength(enum ParamsIOFlag ioFlag) {
   parent->parameters()->ioParamValue(
         ioFlag, name, "strength", &strength, strength /*default value*/, true /*warnIfAbsent*/);
}
...
void InitGauss2DWeightsParams::ioParam_numOrientationsPre(enum ParamsIOFlag ioFlag) {
   if (pre->getLayerLoc()->nf > 1) {
      parent->parameters()->ioParamValue(
            ioFlag, name, "numOrientationsPre", &numOrientationsPre, this->pre->getLayerLoc()->nf);
   }
}

void InitGauss2DWeightsParams::ioParam_deltaThetaMax(enum ParamsIOFlag ioFlag) {
   parent->parameters()->ioParamValue(
         ioFlag, name, "deltaThetaMax", &deltaThetaMax, getDeltaThetaMax());
}
...
float InitGauss2DWeightsParams::calcDthPre() { return PI * thetaMax / (float)numOrientationsPre; }

float InitGauss2DWeightsParams::calcTh0Pre(float dthPre) { return rotate * dthPre / 2.0f; }
