...
{
   handleError(cudaEventCreate(&startEvent));
   handleError(cudaEventCreate(&stopEvent));
   time = 0;
}
...
CudaTimer::~CudaTimer()
{
   handleError(cudaEventDestroy(startEvent));
   handleError(cudaEventDestroy(stopEvent));
}

double CudaTimer::start()
{
   handleError(cudaEventRecord(startEvent, stream));
   return 0;
}

double CudaTimer::stop()
{
   handleError(cudaEventRecord(stopEvent, stream));
   return 0;
}
...
double CudaTimer::accumulateTime(){
   float curTime;
   handleError(cudaEventSynchronize(stopEvent));
   handleError(cudaEventElapsedTime(&curTime, startEvent, stopEvent));
   //Roundoff errors?
   time += curTime;
   return (double) time;
}
