...
// Does 2 things: permutate ordering from [outFeature, ny, nx, inFeature] to [outFeature, inFeature,
// ny, nx]
// Reshapes the matrix if manyScale > 1 to map different "many" kernels into feature dimension
// Coalesced in input
__global__ void CudaPermutePVToCudnn(
      float *dest,
      float *src,
      int outFeatures,
      int ny,
...
__global__ void CudaPermuteCudnnToPV(
      float *dest,
      float *src,
      int outFeatures,
      int ny,
...
void CudaKernel::callPermuteDatastorePVToCudnnKernel(
      int gridSize,
      int blockSize,
      float *pvBuffer,
      float *cudnnBuffer,
...
void CudaKernel::callPermuteGSynPVToCudnnKernel(
      int gridSize,
      int blockSize,
      float *pvBuffer,
      float *cudnnBuffer,
...
void CudaKernel::callPermuteGSynCudnnToPVKernel(
      int gridSize,
      int blockSize,
      float *pvBuffer,
      float *cudnnBuffer,
