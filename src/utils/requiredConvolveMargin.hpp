...
inline int requiredConvolveMargin(int nPre, int nPost, int patchSize) {
   ...
   if (nPre == nPost) {
      ...
      FatalIf(
            ...
            "many-to-one weights but pre is not a power of two times post\n");
      ...
      FatalIf(
            log2densityRatio != std::round(log2densityRatio),
            "one-to-many weights but post is not a power of two times pre\n");
      FatalIf(
            patchSize % densityRatio != 0,
            "one-to-many weights but patch size is not a multiple of post/pre\n");
