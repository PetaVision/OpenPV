...
// vpv_log_debug was deprecated June 16, 2016.  It was only used by functions that themselves are
// deprecated.
// If we decide we want va_list-based functions, add a vprintf method to _Log.
void vpv_log_with_prefix(
      std::ostream &stream,
      const char *prefix,
      const char *file,
      int line,
...
// vpv_log_warn was deprecated June 16, 2016.  It was only used by functions that themselves are
// deprecated.
// If we decide we want va_list-based functions, add a vprintf method to _Log.
...
// vpv_log_error was deprecated June 16, 2016.  It was only used by functions that themselves are
// deprecated.
// If we decide we want va_list-based functions, add a vprintf method to _Log.
...
void pv_log_error(const char *file, int line, const char *fmt, ...) {
   ...
   deprecationWarning
         << "     Use pvWarn or pvErrorNoExit to print to the error stream without exiting."
         << std::endl;
   va_list args;
   va_start(args, fmt);
