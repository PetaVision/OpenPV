...
namespace PV {
   namespace BufferUtils {
      ...
      void nearestNeighborInterp(float const * bufferIn,
      ...
      void bicubicInterp(float const * bufferIn,
            int widthIn,   int heightIn,  int numBands,
            int xStrideIn, int yStrideIn, int bandStrideIn,
            float * bufferOut,
            int widthOut,  int heightOut) {
      ...
      void rescale(Buffer<float> &buffer,
         ...
         std::vector<float> scaledInput(resizedWidth
                                      * resizedHeight
                                      * buffer.getFeatures());
         switch(interpMethod) {
            case BICUBIC:
               bicubicInterp(rawInput.data(),                       
                             buffer.getWidth(),
                             buffer.getHeight(),
                             buffer.getFeatures(),
                             buffer.getFeatures(),
               ...
               break;
            case NEAREST:
               nearestNeighborInterp(rawInput.data(),
                                     buffer.getWidth(),
                                     buffer.getHeight(),
                                     buffer.getFeatures(),
                                     buffer.getFeatures(),
         ...
         switch(rescaleMethod) {
            case CROP:
               buffer.crop(newWidth, newHeight, anchor);
               break;
            case PAD:
               buffer.grow(newWidth, newHeight, anchor);
               break;
         }
      }
