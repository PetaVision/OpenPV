...
class L2NormProbe : public AbstractNormProbe {
  public:
   L2NormProbe(std::shared_ptr<ParamsIO> paramsIO, Communicator const *comm);
   virtual ~L2NormProbe() {}
  ...
  protected:
   ...
   virtual void
   createProbeAggregator(std::shared_ptr<ParamsIO> paramsIO, Communicator const *comm) override;
