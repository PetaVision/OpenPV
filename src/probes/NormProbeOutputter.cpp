...
#include "NormProbeOutputter.hpp"
...
NormProbeOutputter::NormProbeOutputter(std::shared_ptr<ParamsIO> paramsIO, Communicator const *comm) {
   initialize(paramsIO, comm);
}

void NormProbeOutputter::initialize(std::shared_ptr<ParamsIO> paramsIO, Communicator const *comm) {
...
void NormProbeOutputter::printToFiles(ProbeDataBuffer<double> const &storedValues, int numNeurons) {
   ...
   if (getIOMPIBlock()->getRank() == 0) {
      ...
      for (unsigned int n = 0; n < numTimestamps; ++n) {
         ...
         for (int b = 0; b < numBatchElementsInBlock; ++b) {
            ...
            auto &stats      = normsBatch.getValue(bLocal);
            double timestamp = normsBatch.getTimestamp();
            printNorm(returnOutputStream(b), timestamp, numNeurons, globalBatchStart + b, stats);
         }
      }
   }
   else {
   ...
   for (unsigned int n = 0; n < numTimestamps; ++n) {
      for (int b = 0; b < getLocalNBatch(); ++b) {
         ...
         auto &norm                          = normsBatch.getValue(b);
         double timestamp                    = normsBatch.getTimestamp();
         printNorm(returnOutputStream(b), timestamp, numNeurons, b, stats);
      }
   }
#endif // PV_USE_MPI
}
