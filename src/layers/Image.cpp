 ...
 */
Image::Image(unsigned int nxInit, unsigned int nyInit)
{
	Image(nxInit, nyInit, NULL);
}

Image::Image(unsigned int nxInit, unsigned int nyInit, pvdata_t *targetbuf)
{
	if (0 == (nxInit * nyInit)) {
		std::cerr << "Error: Image: Illegal arguments.\n";
		exit(1);
	}
	...
	if ((buf = (pvdata_t *) malloc(nx * ny * sizeof(pvdata_t))) == NULL) {
		std::cerr << "Error: Image: Out of memory.\n";
		exit(1);
	}
	updateImage(0, 0, targetbuf);
}
 ...
 *
 */
int Image::updateImage(float time_step, float dt, pvdata_t *targetbuf)
{
   ...
   static unsigned int posx = 5;
   static unsigned int posy = 10;
 ...
 * Return value: 0 if successful, else non-zero.
 */
int Image::fillImage(pvdata_t val) {
 ...
 * Return value: 0 if successful else non-zero.
 */
int Image::drawMultipleRandomShapes(int n_images)
{
   clearImage();
   srand(time(NULL));
 ...
 */
int Image::drawLine(Point2D origin, unsigned int length, float theta) {

   unsigned int x1 = origin.getX();
   unsigned int y1 = origin.getY();
   unsigned int x2 = approx(x1 + ((length - 1) * cos(deg2rad(theta))));
   unsigned int y2 = approx(y1 + ((length - 1) * sin(deg2rad(theta))));
 ...
 * Return value: None.
 */
int Image::drawLine(Point2D pt1, Point2D pt2) {

   unsigned int x1 = pt1.getX();
   unsigned int y1 = pt1.getY();
   unsigned int x2 = pt2.getX();
   unsigned int y2 = pt2.getY();
 ...
 */
int Image::drawSquare(Point2D origin, unsigned int length, unsigned int theta)
{
 ...
 */
int Image::drawSquare(Point2D pt1, Point2D pt2, Point2D pt3, Point2D pt4)
{
 ...
 *                   theta must be in degrees.
 */
int Image::drawRectangle(Point2D origin, unsigned int lengtha,
   ...
   if ((x1 <= 0) || (x2 <= 0) || (x3 <= 0) || (x4 <= 0) ||
       (y1 <= 0) || (y2 <= 0) || (y3 <= 0) || (y4 <= 0) ||
       (x1 > (int)nx) || (x2 > (int)nx) || (x3 > (int)nx) || (x4 > (int)nx) ||
       (y1 > (int)ny) || (y2 > (int)ny) || (y3 > (int)ny) || (y4 > (int)ny)) {

      std::cout << "Image: Error drawing 2D figure at "; // << origin << ".\n";
      return 1;
   }
 ...
 * Return value: None.
 */
int Image::drawRectangle(Point2D pt1, Point2D pt2,
 ...
 * Return value: None.
 */
int Image::drawQuadrilateral(Point2D pt1, Point2D pt2,
                           Point2D pt3, Point2D pt4) {
   ...
   if ((x1 <= 0) || (x2 <= 0) || (x3 <= 0) || (x4 <= 0) ||
       ...
       (y1 > (int)ny) || (y2 > (int)ny) || (y3 > (int)ny) || (y4 > (int)ny)) {
         ...
         std::cout << "Image: Error drawing 2D figure at ";// << pt1 << ".\n";
         return 1;
      }
 ...
 * ToDo: Rewrite
 */
int Image::drawBresenhamLine(int x1, int y1, int x2, int y2) {
 ...
 * Return value:
 */
inline void Image::mark(unsigned int i, unsigned int j, int value)
{
   *(buf + ((i - 1) + ((j - 1) * nx))) = value;
}
 ...
 * Return value:
 */
inline void Image::mark(unsigned int i, int value)
{
   *(buf + i) = value;
}
 ...
 */
inline pvdata_t Image::getmark(unsigned int i, unsigned int j)
{
   return (*(buf + ((i - 1) + ((j - 1) * nx))));
}
 ...
 * Return value: 0 if successful, non-zero otherwise.
 */
int Image::writeImageToFile(const float time, const unsigned char options)
{
   int err  = 0;
   ...
   if (istifon) {
      ...
      err |= tiff_write_file(tiffilename, buf, nx, ny);
   }
   if (istxton) {
 ...
 * Return value: 0 if successful, non-zero otherwise.
 */
int Image::writeImageToTxt(const char *filename)
{
   FILE *txtfile = NULL;
   unsigned int i, j;
 ...
 * Return value: 0 if successful, non-zero otherwise.
 */
int Image::writeImageToBin(const char *filename)
{
   FILE *binfile = NULL;
   unsigned int i, j;
...
/*
 * Description: Copy the data from buf maintained by Image class to
 ...
 * Return value: 0 if successful, non-zero otherwise.
 */
int Image::copyImage(pvdata_t *targetbuf)
{
	for (unsigned int i = 0; i < (nx * ny); i++) {
		targetbuf[i] = buf[i];
	}
 ...
 */
int Image::threewaytoss(double probBack, double probStay, double probForward)
{
 ...
 */
inline double Image::deg2rad(int angleInDegrees)
{
 ...
 *               else undefined.
 */
inline int Image::approx(double temp)
{
   return (int)(temp + 0.5);
}
