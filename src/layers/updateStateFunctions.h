...
#pragma omp parallel for schedule(static)
...
#endif // PV_USE_CUDA
   {
      ...
      pvdata_t currentDrive = (1.0f - exp_tau) * ((GSynErrorBatch[k] - GSynError2Batch[k])
                                                  + selfInteract * activityBatch[kex]);
      VBatch[k]         = exp_tau * VBatch[k] + currentDrive + LCAMomentumRate * prevDriveBatch[k];
      prevDriveBatch[k] = currentDrive;
   }
...
int applyGSyn_ANNWhitenedLayer(
   ...
   MEM_GLOBAL pvdata_t *GSynAveSquaredInput =
         &GSynHead[2 * nbatch * numNeurons]; // un-whitened input
#ifndef PV_USE_CUDA
#ifdef PV_USE_OPENMP_THREADS
#pragma omp parallel for schedule(static)
#endif
...
int setActivity_AccumulateLayer(
      ...
      int dn,
      int up) {
   // static inline int setActivity_HyPerLayer(int numNeurons, pvdata_t * A, pvdata_t * V, int nx,
...
int setActivity_GapLayer(
      ...
      int orig_dn,
      int orig_up,
      MEM_GLOBAL pvdata_t *checkActive,
      float ampSpikelet) {
   int kbatch;
#ifndef PV_USE_CUDA
#ifdef PV_USE_OPENMP_THREADS
