...
namespace PV {
   ...
   Buffer<float> PvpLayer::retrieveData(std::string filename, int batchIndex) {
      ...
      // frame in the batch.
      if(mPvpFrameCount == -1) {
         FileStream headerStream (filename.c_str(),
                                  std::ios_base::in
                                | std::ios_base::binary,
                                  false);
      ...
      Buffer<float> result(mInputNx,
                           mInputNy,
                           mInputNf);

      switch (mFileType) {
         case PVP_NONSPIKING_ACT_FILE_TYPE:
            BufferUtils::readFromPvp<float>(filename.c_str(),
                                            &result,
                                            frameNumber);
            break;
         case PVP_ACT_SPARSEVALUES_FILE_TYPE:
            BufferUtils::readSparseFromPvp<float>(filename.c_str(),
                                                  &list,
                                                  frameNumber,
                                                  &sparseTable);
         ...
         case PVP_ACT_FILE_TYPE:
            ...
            BufferUtils::readSparseBinaryFromPvp<float>(filename.c_str(),
                                                        &list,
                                                        frameNumber,
                                                        {1},
                                                        &sparseTable);
