...
static inline int update_V(int n, float phi[], float phi_h[], float V[], float f[]);
    ...
    for (k = 0; k < N; k += CHUNK_SIZE)
      {
        ...
        update_phi(CHUNK_SIZE, N, &phi[k], &x[k], &y[k], &o[k], x, y, o, f, EXCITE_R2, SIG_C_D_x2, SIG_C_P_x2, COCIRC_SCALE);
...
int pv_layer_finish_update(PVLayer* l, int time_index)
  {
#ifdef INHIBIT_ON
    update_V(N, l->phi, l->phi_h, l->V, l->f);
    update_f(l->n_neurons, l->V, l->f); 
    inhibit_update(l);
 #else 
    float phi_h[N]={0.0}; 
    update_V(N, l->phi, phi_h, l->V, l->f);
    update_f(l->n_neurons, l->V, l->f);  
#endif
...
static inline int update_V(int n, float phi[],float phi_h[], float V[], float f[])
  {
	...
	#ifdef INHIBIT_ON
	V[i] += DT_d_TAU*(r + phi[i] - V[i] -SCALE_INH*phi_h[i]);
	phi_h[i]=0.0;
	#else
