...
class HyPerCol : public Subject {
  ...
  private:
   ...
   /**
    * @brief dtAdaptController: Obsolete.  Adaptive timescale parameters have moved to
    * AdaptiveTimeScaleProbe.
    */
   virtual void ioParam_dtAdaptController(enum ParamsIOFlag ioFlag);

   /**
     * @brief dtAdaptFlag: Obsolete.  Set the AdaptiveTimeScaleProbe useAdaptiveTimeScales parameter
    * instead.
     */
   virtual void ioParam_dtAdaptFlag(enum ParamsIOFlag ioFlag);

   /**
    * @brief useAdaptMethodExp1stOrder: Obsolete.  Exp1stOrder is currently the only supported
    * adaptive timestep scheme.
    */
   virtual void ioParam_useAdaptMethodExp1stOrder(enum ParamsIOFlag ioFlag);

   /**
    * @brief mDtAdaptTriggerLayerName: Obsolete.  This parameter is now handled by
    * AdaptiveTimeScaleProbe.
    */
   ...
   /**
    * @brief dtScaleMin: Obsolete.  This parameter is now handled by AdaptiveTimeScaleProbe, as
    * baseMin.
    */
   virtual void ioParam_dtScaleMin(enum ParamsIOFlag ioFlag);

   /**
    * @brief dtChangeMax: Obsolete.  This parameter is now handled by AdaptiveTimeScaleProbe, as
    * tauFactor.
    */
   virtual void ioParam_dtChangeMax(enum ParamsIOFlag ioFlag);

   /**
    * @brief dtChangeMin: Obsolete.  This parameter is now handled by AdaptiveTimeScaleProbe, as
    * growthFactor.
    */
   ...
   /**
    * @brief writeTimeScaleFieldnames: Obsolete.  This parameter is now handled by
    * AdaptiveTimeScaleProbe.
    */
   ...
   /**
    * @brief writeProgressToErr: Whether to print timestep progress to the error stream instead of
    * the output stream
    */
   ...
   /**
    * @brief mFilenamesContainLayerNames: Specifies if layer names gets printed out to output
   ...
   /**
    * @brief defaultInitializeFromCheckpointFlag: Flag to set the default for layers and
    * connections.
   ...
   /**
    * @brief checkpointWriteDir: If checkpointWrite is set, specifies the output checkpoint
    * directory.
    */
   virtual void ioParam_checkpointWriteDir(enum ParamsIOFlag ioFlag);

   /**
    * @brief mCheckpointWriteTriggerMode: If checkpointWrite is set, specifies the method to
    * checkpoint.
   ...
   /**
    * @brief checkpointWriteStepInterval: If checkpointWrite on step, specifies the number of steps
    * between checkpoints.
    */
   virtual void ioParam_checkpointWriteStepInterval(enum ParamsIOFlag ioFlag);

   /**
    * @brief checkpointWriteTimeInteval: If checkpointWrite on time, specifies the amount of
    * simulation time between checkpoints.
    */
   virtual void ioParam_checkpointWriteTimeInterval(enum ParamsIOFlag ioFlag);

   /**
    * @brief checkpointWriteClockInteval: If checkpointWrite on clock, specifies the amount of clock
   ...
   /**
    * @brief checkpointWriteClockInteval: If checkpointWrite on clock, specifies the units used in
    * checkpointWriteClockInterval.
    */
   virtual void ioParam_checkpointWriteClockUnit(enum ParamsIOFlag ioFlag);

   /**
    * @brief deleteOlderCheckpoints: If checkpointWrite, specifies if the run should delete older
   ...
   /**
    * @brief supressLastOutput: If checkpointWrite, specifies if the run should supress the final
    * written checkpoint for the end of the run.
    */
   virtual void ioParam_suppressLastOutput(enum ParamsIOFlag ioFlag);

   /**
    * If checkpointWrite is true and this flag is true, connections' checkpointWrite method will
   ...
   /**
    * @brief If checkpointWrite is true, checkpointIndexWidth specifies the minimum width for the
    * step number appearing in the checkpoint directory.
    * @details If the step number needs fewer digits than checkpointIndexWidth, it is padded with
    * zeroes.  If the step number needs more, the full
    * step number is still printed.  Hence, setting checkpointWrite to zero means that there are
   ...
   /**
    * @brief errorOnNotANumber: Specifies if the run should check on each timestep for nans in
    * activity.
    */
...
// Sep 26, 2016: HyPerCol methods for parameter input/output have been moved to PVParams.
  ...
  private:
   ...
   Secretary *mSecretary =
         nullptr; // manages checkpointing and, eventually, will manage outputState output.
