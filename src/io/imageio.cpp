...
int getImageInfoPVP(const char *filename, PV::Communicator *comm, PVLayerLoc *loc) {
   ...
   pvDebug().printf(
         "[%2d]: nxProcs==%d nyProcs==%d icRow==%d icCol==%d\n",
         comm->commRank(),
         nxProcs,
         nyProcs,
         icRow,
...
int gatherImageFilePVP(
   ...
   if (rank == rootproc) {
      ...
      if (numWrite != numParams) {
         pvError().printf(
               "gatherImageFilePVP error writing the header.  fwrite called with %d parameters; %d "
               "were written.\n",
               numParams,
               numWrite);
      }
   ...
   // buf is a nonextended buffer.  Image layers copy buf into the extended data buffer by calling
   // Image::copyFromInteriorBuffer
   if (rank == rootproc) {
      PV::PV_fclose(pvstream);
      pvstream = NULL;
   }
