...
int writeRandState(
   ...
   for (int b = 0; b < loc->nbatch; b++) {
      ...
      status =
            gatherActivity(pvstream, comm, rootproc, randStateBatch, loc, isExtended /*extended*/);
   }
   if (rank == rootproc) {
      PV_fclose(pvstream);
      pvstream = NULL;
...
int readRandState(
   ...
   for (int b = 0; b < loc->nbatch; b++) {
      ...
      status =
            scatterActivity(pvstream, comm, rootproc, randStateBatch, loc, isExtended /*extended*/);
   }
   if (rank == rootproc) {
      PV_fclose(pvstream);
      pvstream = NULL;
