...
int main(int argc, char* argv[])
{
	...
	const float REL_POS_X0[2] =
	{ 1.0 / 2.0, 1.0 / 2.0 };
	const float REL_POS_Y0[2] =
	{ 1.0 / 2.0, 1.0 / 2.0 };
	const float REL_RADIUS[2] =
	{ 1.0 / 5.0, 1.0 / 5.0 };

	const char input_path[64] = "./input/circ_pix_";

	int k, i;
	int u[NUM_CIRC];
	int j, jj, jjj, p; //indices for x and y initialization

	eventtype_t f[NX*NY]; // CHG for pixels: no O dimension
	float dx, dy, r, r2;
	int nrows = (int) 1;
	...
	if (num == NULL)
		...
		fwrite(&NUM_CIRC, sizeof(int), 1, num);
	...
	for (i = 0; i < NUM_CIRC; i++)
	{
		...
		for (k = 0; k < N; k+=NO)
		{
			...
			r = rand() * INV_RAND_MAX;
			f[k/NO] = (r < clutter_prob) ? I_MAX : 0.0;
			...
			if (r2 > r2_min && r2 < r2_max)
			{
				...
				if (1) // if (t == kk)//&& f[k] == 0.0)
				{
					f[k / NO] = I_MAX; // CHG for pixels: divide out the orientation, so just X and Y now
		...
		fclose(fo);
		if (input_file != NULL)
			fwrite(f, sizeof(float), N/NO, input_file);
