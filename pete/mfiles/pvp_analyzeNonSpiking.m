...
global output_path spiking_path twoAFC_path activity_path
...
num_single_trials = 5;
num_trials =  116; %( TRAINING_FLAG <= 0 ) * 999; % %
if ~TOPDOWN_FLAG
    first_trial = 1;
else
    first_trial = 2;
...
machine_path = '/Users/pschultz/Workspace/NMC/eclipse-workspace/';
%machine_path = '/nh/home/gkenyon/workspace/';
global target_path
% target_path = [];
target_path = [machine_path 'GenerativeLateral/output/target/0001/test']; %, FC_STR];
...
if num_trials > num_single_trials || RAW_HIST_FLAG
    distractor_path = [machine_path, ...
        'GenerativeLateral/output/distractor/0001/test']; %, FC_STR];
else
    distractor_path = [];
end%%if
...
for i_conn = plot_weights
    ...
    elseif i_conn == N_CONNECTIONS + 1
        ...
        end%%if % write_pvp_kernel_flag
    else
        continue;
    end%%if  % i_conn < N_CONNECTIONS + 1
