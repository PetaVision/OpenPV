...
void * customgroup(const char * keyword, const char * name, HyPerCol * hc) {
	   ...
	   char * message = NULL;
	   bool errorFound;
	   if( !strcmp(keyword, "FeedbackDelayTestProbe") ) {
	      ...
	      if( !errorFound ) {
	         ...
	         if( filename ) {
	            addedProbe = (LayerProbe *) new FeedbackDelayTestProbe(filename, targetlayer, message);
	         }
	         else {
	            addedProbe = (LayerProbe *) new FeedbackDelayTestProbe(targetlayer, message);
	         }
	         if( !addedProbe ) {
