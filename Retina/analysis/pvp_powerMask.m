function [xcorr_struct] = ...
  ...
  for i_mode = 1 : xcorr_struct.num_modes  % 1 == peak, 2 = mean
    ...
    if xcorr_struct.calc_power_mask
      ...
      numel_target = numel(cell2mat(target_struct.target));
      numel_clutter = numel(target_struct.clutter);
      disp(["======= layer is",num2str(layer)," i_mode is",num2str(i_mode)]);
      while xcorr_struct.num_power_mask(layer,i_mode) > ...
	    xcorr_struct.num_sig * ...
	    ( numel_target + numel_clutter )

        num_power_sig = num_power_sig + 0.1;
        xcorr_struct.power_mask{layer, i_mode} = ...
            find( power_array{layer, i_mode} > ...
		 ( mean_power + num_power_sig * std_power ) );
        xcorr_struct.power_mask{layer,i_mode} = ...
        ...
        disp( ['num_power_mask(', num2str(layer), ') = ', ...
	       num2str(xcorr_struct.num_power_mask(layer,i_mode)), ' > ', ...
               num2str( mean_power + num_power_sig * std_power ) ] );
        disp(["** xcorr_struct.num_power_mask(layer,i_mode) is  ", \
	      num2str(xcorr_struct.num_power_mask(layer,i_mode))]);
        disp(["** xcorr_struct.num_sig is  ",num2str(xcorr_struct.num_sig)]);
	disp(["** numel_target is  ",num2str(numel_target)]);
	disp(["** numel_clutter is  ",num2str(numel_clutter)]);
	disp(["** num_power_sig is  ",num2str(num_power_sig)]);
      endwhile %%
    else
      xcorr_struct.power_mask{layer,i_mode} = ...
