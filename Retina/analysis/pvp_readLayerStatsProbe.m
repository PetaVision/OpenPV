...
function [time,Avg] = pvp_readLayerStatsProbe(filename)

%Finds vector of values for time, N, total, Min, Avg, Max; outputs time and Avg
filename = ["../output/stats",filename,".txt"]
myfile = fopen(filename,"r");
if myfile == -1
   error("No input file");
end %if

%Count number of lines in .txt file, which is equivalent to time
...
%Find the values in the file for each vector
