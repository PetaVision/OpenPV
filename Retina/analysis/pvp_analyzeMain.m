...
global NUM_PROCS 
NUM_PROCS = 2;                  % two is max on the MAC    
...
machine_path = '/Users/gerdjkunde/';
matlab_path = [machine_path];
workspace_path = [machine_path, 'Documents/workspace/'];
project_path = [workspace_path, 'gjkunde/'];

global OUTPUT_PATH SPIKE_PATH
SPIKE_PATH = [project_path, 'output/'];
OUTPUT_PATH = [project_path, 'octave/'];

image_path = [matlab_path, '']; %% fix
image_filename = [image_path ''];     %% fix
target_filename{1} = [image_path '']; %% fix

main_file = [project_path, 'pv.cpp'];

params_file = [project_path, 'input/params.pv'];

global pvp_order
pvp_order = 1;

%% set duration of simulation, if known (determined automatically otherwise)
BEGIN_TIME =0000.0  % (msec) start analysis here, used to exclude start up artifacts
END_TIME   =2000.0

%% stim begin/end times (msec) relative to begining/end of each epoch
STIM_BEGIN_TIME = 200; %%200.0;  % relative to begining of epoch, must be > 0
STIM_END_TIME   =-100; %%-200.0;  % relative to end of epoch, must be <= 0
...
global N_LAYERS   %% this comes from pvp_layerID.m
[layerID, layerIndex] = pvp_layerID(); % edit this for nlayers
num_layers = N_LAYERS;
read_spikes = [13]; %[layerIndex.l1, layerIndex.l1inh];% list of spiking layers whose spike train are to be analyzed
...
if exist (image_filename,"file") && exist(target_filename,"file")
  ...
  [row_pixels, col_pixels] = meshgrid(30:40, 30:90);
  target_struct.target{1} = ...
      zeros(11*61,1);
  target_struct.target{1}(:) = ...
      sub2ind([NROWS_image,NCOLS_image], row_pixels(:), col_pixels(:));
;
%%      [1:num_pixels]';
  target_struct.clutter = zeros(11*61,1);
  [row_pixels, col_pixels] = meshgrid(150:160, 30:90);
...
%% display top ranked cells
  ...
  if rate_array_tmp(1) == 0     % take out non spiking
				% layers
...
xcorr_freqs = 1000*(1/DELTA_T)*(0:2*max_lag)/(1+2*max_lag);
