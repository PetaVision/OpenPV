...
public:
   ...
   virtual ParamGroupType getGroupType(char const * keyword) {
      ...
      else if (!strcmp(keyword, "ParameterSweepTestProbe")) { result = ProbeGroupType; }
      return result;
   }
   ...
   virtual BaseProbe * createProbe(char const * keyword, char const * name, HyPerCol * hc) {
      ...
      else if (!strcmp(keyword, "ParameterSweepTestProbe")) {
         addedProbe = new ParameterSweepTestProbe(name, hc);
      }
      if (addedProbe==NULL) {
